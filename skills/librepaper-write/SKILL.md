...
For an uncertain write, retry the original call unchanged only when that is the supported idempotent recovery path. `document_result` is an operation-state lookup, not a retained receipt: an admitted write may remain unresolved, and an identical retry cannot make an unknown effect safe to replay. Treat an unknown effect as unknown and inspect without inventing a new operation ID. A bounded reread may refresh expired read context, but never assumes the same passage or current revision and never replays an uncertain write. On conflict, read fresh evidence and preserve the intended occurrence; never silently swap the old revision for the newest one.
