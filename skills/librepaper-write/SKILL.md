...
`document_propose` accepts `view_id`, `operation: {epoch, id}`, and `patches: [{range_id, replacement, note}]`. Choose a fresh operation ID for each new intent. Use `find` only for a unique exact substring within the captured range. Duplicate occurrences require a more precise source range.
...
For an uncertain write, retry the original call unchanged only when that is the supported idempotent recovery path. `document_result` can recover a retained outcome using the original operation identity while its epoch is live. Report only effects confirmed by that outcome. Missing, legacy, or expired evidence remains unknown; an identical retry cannot make an unknown effect safe to replay. Treat an unknown effect as unknown and inspect without inventing a new operation ID. A bounded reread may refresh expired read context, but never assumes the same passage or current revision and never replays an uncertain write. On conflict, read fresh evidence and preserve the intended occurrence; never silently swap the old revision for the newest one.
