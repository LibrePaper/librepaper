...
def reject(name, sql, parameters=(), error_contains=None):
    global checks
    db.execute("SAVEPOINT invalid_row")
    try:
        try:
        ...
        db.execute("ROLLBACK TO invalid_row")
        db.execute("RELEASE invalid_row")


def accept(name, sql, parameters=()):
    db.execute("SAVEPOINT valid_row")
    try:
        db.execute(sql, parameters)
        check(True, name)
        ...
        db.execute("ROLLBACK TO valid_row")
        db.execute("RELEASE valid_row")
...
def boundary_probe(name, sql, parameters=()):
    ...
    db.execute("SAVEPOINT application_boundary")
    try:
        db.execute(sql, parameters)
        boundary_checks += 1
    except sqlite3.Error as error:
        ...
        db.execute("ROLLBACK TO application_boundary")
        db.execute("RELEASE application_boundary")
...
check(db.execute("SELECT count(*) FROM sqlite_schema WHERE type='trigger'").fetchone()[0] == 0,
      "no triggers")
check(all(row[5] for row in db.execute("PRAGMA table_list") if row[0] == "main" and row[1] in expected),
      "all tables strict")
...
for kind, required_scope in sorted(scope_by_kind.items()):
    ...
    db.execute("SAVEPOINT deadline_matrix")
    ...
    try:
        ...
        db.execute("ROLLBACK TO deadline_matrix")
        db.execute("RELEASE deadline_matrix")
...
db.execute("SAVEPOINT title_release")
...
db.execute("ROLLBACK TO title_release")
db.execute("RELEASE title_release")
