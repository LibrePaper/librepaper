...
Status: implemented. This specification describes the storage architecture the
application ships; the migrations in `crates/librepaper/migrations/postgres/`
are the normative DDL. Provider-specific deployment acceptance remains an
...
### 5.3 Assets and source versions
...
one new compressed Loro snapshot, and conditionally updates `document_bases`
...
### 7.4 Suggestions and agent writes

A suggestion or agent captures a source version or CRDT version vector, the
...
## 8. Realtime collaboration and horizontal processes
...
## 17. Performance and cost acceptance
