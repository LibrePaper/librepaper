...
`live_root` protects a physical object required by acknowledged live state, including dependencies named by live journal entries. `publication_root` protects every physical object in the current rendered bundle. These are concrete root sets, not arbitrary bit flags for future subsystems. There is no live reference hidden only inside an unparsed operation plan. The partial `objects_live_roots(document_id,kind,id)` index selects available live roots when replacing an acknowledged journal dependency set; this transition must not scan retained object history. Clearing a superseded journal root is independent of checkpoint/publication references, which continue to protect the object through their own GC checks.
...
The revised DDL was executed in an isolated database using SQLite 3.51.2: twelve tables, all STRICT, zero triggers, successful integrity/FK checks. The accompanying [validate-catalog-v2.py](validate-catalog-v2.py) passed **187 checks** on SQLite 3.53.1, including the complete kind/scope and deadline matrices, scoped erasure uniqueness, all three natural replay query plans, counter ceilings, settled-object receipt cleanup, title/slug release semantics, and initialization rollback.
