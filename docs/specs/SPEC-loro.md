...
Steps 1 to 4 are **one atomic server operation**. The intermediate state contains
...
What it is *about* is `room::annotation::OriginalAnchor`: the `document_updates`
row that made the quoted text durable (`source_sequence`) and the Loro frontier
that state was measured against (`frontier`), together with either a UTF-16
range of one file, named by its stable `files` key, or the document as a whole
(SPEC-server-is-a-log.md §8.2). The server works it out once, when the comment
is made, in `room::locate` -- from the words the browser saw, because a browser
has no source to send offsets into and a reader of a published rendering has no
source at all -- and nothing writes to it afterwards.

Where that passage *is* is `room::resolve`'s `DerivedAttachment`: a cache,
keyed by the tree digest it was computed against, recomputed on every edit.
Cursors captured at creation, when that source state was live, are what it
...
- **Update log** -- `document_updates` rows hold Loro update bytes. The row
  shape (framing, the per-row `vector` column, what a sequence number
  identifies) was rewritten by the later "server is a log" cutover; see
  SPEC-server-is-a-log.md §4.2. That rewrite changed how a row is produced and
  read, not that Loro update bytes are what it holds.
...
`doc-sync` itself did not last: SPEC-server-is-a-log.md §6.1 removes it, since
join and catch-up are answered from stored row vectors rather than from a live
document (§6.2 there).
...
4. ~~Reconsider checkpoint density~~ -- overtaken by a later decision rather
   than resolved as framed. This item assumed a timed cadence of automatic
   checkpoints and asked only how dense it should be. The 2026-09-18 "versions
   are asked for" cutover removed the cadence itself: a label is created only
   when a person or a source-producing command asks for one, never on a timer,
   so there is no burst to bound and no `CHECKPOINT_DEFER_SECONDS` any more.
   What does not move is the reason archives are not thinned: contract 6 is
   satisfied *by the Loro oplog*, and an archive is what makes recovery
   format-independent and Loro-independent (contract 8, SPEC-server-is-a-log.md
   §8.5). §7.3 puts an archive fetch at 0.08 ms against 3.9 ms to check out an
   old version, so an archive is also the fastest point-in-time retrieval by a
   factor of about fifty -- now produced on request from a label's frontier,
   not written eagerly when the label is made.
