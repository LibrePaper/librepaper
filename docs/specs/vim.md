...
`docs/specs/sync.md` for what the editor's undo means on a shared document.
...
meaning that `docs/specs/sync.md` fixes and a second undo stack would break
...
4. The undo check against the sync spec, and the browser smoke in
   `web/tools/browser-smoke.mjs`: turn Vim on, type `ihello<Esc>`, expect
