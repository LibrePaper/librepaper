...
The Overleaf research note (`docs/research/`, §1 and §2) sets the bar a
...
to `docs/specs/history.md`'s rule that nothing derived is stored, and it is
...
`crates/engine/src/diagnostic.rs` defines, so the underline and the gutter mark are the
...
checkpoint's SHA in the sense of `docs/specs/history.md`, and its bytes are the
