...
synchronization. Before the first bundle, metadata returns null.
...
`GET /api/documents/:slug/bundle` returns `{bundle: null}` or a record
...
`v2.<issued_at_ms>.<32 lowercase hex random>`. Generate one key per bundle
...
The bundle ID is generated by the server independently of this key.

Publishing uses these operations:

1. `POST /api/documents/:slug/bundle/prepare` with
   `{manifest, expected_bundle_id}` returns `{missing: [sha256, ...]}`.
2. `PUT /api/documents/:slug/bundle/objects/:sha256` uploads a missing
...
3. `POST /api/documents/:slug/bundle/activate` sends the same prepare body
   and idempotency key, and returns `{bundle: ...}` after activation.
...
The server supplies bundle identity, timestamp and attribution.

A conflicting expected bundle returns 409. A changed descriptor cannot
...
Bundle delivery and object uploads share the deployment's artifact-transfer
...
edit. It measures bundle metadata separately from object bodies:

| Explicit bundle | Object upload | Image upload | Current object storage | Reader object download | Cached objects reused |
| --- | ---: | ---: | ---: | ---: | ---: |
...
Each row uses two bundle metadata requests plus one explicit reader
