...
fn decode_recipe(bytes: &[u8]) -> Result<RecipeInfo> {
    ...
    let expected=60usize.checked_add(count.checked_mul(36).ok_or_else(||Error::Invalid("source recipe size overflow".into()))?).ok_or_else(||Error::Invalid("source recipe size overflow".into()))?;
...
fn convert_document(source_root: &Path, target_root: &Path, source: &Connection, target: &mut Connection, doc: &SourceDocument, progress: &mut DocumentProgress) -> Result<()> {
    ...
    progress.cursor="content-and-checkpoints".into(); progress.checkpoints=converted.len() as u64; progress.objects=all_objects.len() as u64; progress.bytes=all_objects.values().map(|o|o.bytes.len() as u64).sum(); progress.mapping_digest=sha256(mapping.as_bytes()); let _=closure_count;
    Ok(())
}
