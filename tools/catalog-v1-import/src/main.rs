...
#[derive(Parser, Debug)]
#[command(
    name = "catalog-v1-import",
    about = "Offline v1 to v2 catalog converter"
)]
struct Args {
...
impl Lock {
        ...
        let file = OpenOptions::new()
            ...
            .map_err(|e| {
                Error::Invalid(format!(
                    "source writer lock {} must already exist: {e}",
                    path.display()
                ))
            })?;
        file.try_lock_exclusive().map_err(|e| {
            Error::Invalid(format!(
                "source deployment is not stopped; could not acquire {}: {e}",
                path.display()
            ))
        })?;
        Ok(Self(file))
    ...
    fn target(path: &Path) -> Result<Self> {
        let parent = path
            .parent()
            .ok_or_else(|| Error::Invalid("target lock has no parent".into()))?;
        ...
        file.try_lock_exclusive().map_err(|e| {
            Error::Invalid(format!(
                "target deployment is already in use; could not acquire {}: {e}",
                path.display()
            ))
        })?;
        Ok(Self(file))
...
fn parse_time(value: &str, field: &str) -> Result<i64> {
    ...
    if value.is_empty() {
        return Err(Error::Invalid(format!("{field} is empty")));
    }
    if let Ok(seconds) = value.parse::<i64>() {
        return seconds
            .checked_mul(1000)
            .ok_or_else(|| Error::Invalid(format!("{field} overflows milliseconds")));
    }
...
fn read_lock_path(root: &Path) -> PathBuf {
    root.join("state").join("writer.lock")
}
fn catalog_path(root: &Path) -> PathBuf {
    root.join("catalog.db")
}
fn objects_path(root: &Path) -> PathBuf {
    root.join("objects")
}
...
fn read_object(root: &Path, key: &str) -> Result<Vec<u8>> {
    ...
    let metadata = fs::symlink_metadata(&path)
        .map_err(|e| Error::Invalid(format!("missing source object {key}: {e}")))?;
    if metadata.file_type().is_symlink() {
        return Err(Error::Invalid(format!("source object is a symlink: {key}")));
    }
    Ok(fs::read(path)?)
}
...
fn validate_document_id(value: &str) -> Result<()> {
    ...
    {
        return Err(Error::Invalid(format!(
            "invalid document storage ID `{value}`"
        )));
    }
    Ok(())
}
...
fn write_object(root: &Path, key: &str, bytes: &[u8]) -> Result<()> {
    ...
    if path.exists() {
        let metadata = fs::symlink_metadata(&path)?;
        if metadata.file_type().is_symlink() {
            return Err(Error::Invalid(format!("target object is a symlink: {key}")));
        }
        let old = fs::read(&path)?;
        if old != bytes {
            return Err(Error::Invalid(format!(
                "target object collision with different bytes: {key}"
            )));
        }
        return Ok(());
    }
        ...
        Err(e) if e.kind() == io::ErrorKind::AlreadyExists => {
            ...
            if existing == bytes {
                match fs::hard_link(&temp, &path) {
                    ...
                    Err(link) if link.kind() == io::ErrorKind::AlreadyExists => {
...
fn write_file_no_replace(path: &Path, bytes: &[u8]) -> Result<()> {
    ...
    if path.exists() {
        ...
        if metadata.file_type().is_symlink() || !metadata.is_file() {
            return Err(Error::Invalid(format!(
                "target secret path is not a regular file: {}",
                path.display()
            )));
        }
        if fs::read(path)? != bytes {
            return Err(Error::Invalid(format!(
                "target secret collision with different bytes: {}",
                path.display()
            )));
        }
        return Ok(());
...
fn copy_secrets(source_root: &Path, target_root: &Path) -> Result<u64> {
    ...
    let target = target_root.join("secrets");
    ...
    while let Some(directory) = stack.pop() {
        for entry in fs::read_dir(&directory)? {
            ...
            if !metadata.is_file() {
                return Err(Error::Invalid(format!(
                    "source secret is not a regular file: {}",
                    path.display()
                )));
            }
            let relative = path
                .strip_prefix(source_root)
                .map_err(|_| Error::Invalid("secret path escaped source root".into()))?;
            let destination = target_root.join(relative);
            reject_path_symlinks(target_root, &destination)?;
            let bytes = fs::read(&path)?;
            write_file_no_replace(&destination, &bytes)?;
            copied = copied
                .checked_add(1)
                .ok_or_else(|| Error::Invalid("secret count overflow".into()))?;
        }
    }
    Ok(copied)
}
...
fn reject_path_symlinks(root: &Path, path: &Path) -> Result<()> {
    let relative = path
        .strip_prefix(root)
        .map_err(|_| Error::Invalid("object path escaped root".into()))?;
    ...
    for component in relative.components() {
        ...
        if let Ok(metadata) = fs::symlink_metadata(&current) {
            if metadata.file_type().is_symlink() {
                return Err(Error::Invalid(format!(
                    "symlink ancestor in object path: {}",
                    current.display()
                )));
            }
        }
...
fn catalog_digest(path: &Path) -> Result<String> {
...
fn source_physical_digest(root: &Path) -> Result<String> {
    ...
    while let Some(directory) = stack.pop() {
        for entry in fs::read_dir(&directory)? {
            ...
            if metadata.file_type().is_symlink() {
                return Err(Error::Invalid(format!(
                    "source deployment contains a symlink: {}",
                    path.display()
                )));
            }
            if metadata.is_dir() {
                ...
                let relative = path
                    .strip_prefix(root)
                    .map_err(|_| Error::Invalid("source file escaped deployment root".into()))?;
                files.push((relative.to_string_lossy().replace('\\', "/"), path));
            } else {
                return Err(Error::Invalid(format!(
                    "source deployment entry is not a regular file or directory: {}",
                    path.display()
                )));
            }
        }
    ...
    for (relative, path) in files {
        ...
        loop {
            ...
            length = length
                ...
                .ok_or_else(|| Error::Invalid("source physical file length overflow".into()))?;
            if length > MAX_SOURCE_FILE_BYTES {
                return Err(Error::Invalid(format!(
                    "source file {} exceeds {} byte limit",
                    relative, MAX_SOURCE_FILE_BYTES
                )));
            }
...
fn schema_fingerprint(connection: &Connection) -> Result<String> {
    schema_fingerprint_filtered(connection, false)
}
...
fn schema_fingerprint_filtered(connection: &Connection, filter_runtime: bool) -> Result<String> {
    ...
    for row in statement.query_map([], |row| {
        ...
        if filter_runtime
            && V1_RUNTIME_TABLES
                .iter()
                .any(|table| name == *table || sql.contains(&format!("CREATE TABLE {table}")))
        {
            continue;
        }
...
fn require_columns(connection: &Connection, table: &str, columns: &[&str]) -> Result<()> {
    ...
    for column in columns {
        if !found.contains(*column) {
            return Err(Error::Invalid(format!(
                "v1 table {table} lacks required column {column}"
            )));
        }
    }
    Ok(())
...
fn validate_source(connection: &Connection) -> Result<(String, String)> {
    ...
    if version != 0 && version != V1_USER_VERSION {
        return Err(Error::Invalid(format!(
            "unsupported source user_version {version}; expected v1"
        )));
    }
    let required: &[(&str, &[&str])] = &[
        ...
        (
            "links",
            &[
                "slug", "role", "hash", "sealed", "label", "budget", "since", "until", "key_id",
            ],
        ),
    ...
    if foreign.query([])?.next()?.is_some() {
        return Err(Error::Invalid(
            "source foreign_key_check reported violations".into(),
        ));
    }
    let expected = Connection::open_in_memory()?;
    expected.execute_batch(V1_DDL)?;
    let expected_fp = schema_fingerprint_filtered(&expected, true)?;
    let actual_fp = schema_fingerprint_filtered(connection, true)?;
    if expected_fp != actual_fp {
        return Err(Error::Invalid(format!("source schema fingerprint {actual_fp} does not match supported v1 fixture {expected_fp}; inspect required columns and migrate with the v1 binary first")));
    }
...
fn target_empty_or_resume(root: &Path, resume: bool) -> Result<()> {
    ...
    if resume && root.join(MANIFEST_FILE).is_file() {
        return Ok(());
    }
    Err(Error::Invalid(format!(
        "target {} is nonempty; use --resume only with a conversion manifest",
        root.display()
    )))
}
...
fn source_documents(connection: &Connection) -> Result<Vec<SourceDocument>> {
    ...
    while let Some(row) = rows.next()? {
        ...
        docs.push(SourceDocument {
            ...
            published_at: if published.trim().is_empty() {
                None
            } else {
                Some(parse_time(&published, "documents.published_at")?)
            },
            ...
            main_path: row.get(11)?,
            last_publication_id: row.get(12)?,
            pending_publication: row.get(13)?,
        });
    }
    Ok(docs)
}
...
fn active_key(connection: &Connection, explicit: Option<&str>) -> Result<String> {
    if !has_table(connection, "link_keyring")? {
        if explicit.is_some() {
            return Err(Error::Invalid(
                "--active-link-key is accepted only when source key metadata is ambiguous".into(),
            ));
        }
        return Ok("legacy".to_string());
    }
    let mut statement = connection
        .prepare("SELECT key_id FROM link_keyring WHERE status='primary' ORDER BY key_id")?;
    let candidates = statement
        .query_map([], |row| row.get::<_, String>(0))?
        .collect::<rusqlite::Result<Vec<_>>>()?;
    let available: Vec<String> = connection
        .prepare("SELECT key_id FROM link_keyring WHERE status <> 'retired' ORDER BY key_id")?
        .query_map([], |row| row.get::<_, String>(0))?
        .collect::<rusqlite::Result<Vec<_>>>()?;
...
fn inspect_unresolved(connection: &Connection) -> Result<Vec<String>> {
    ...
    let checks = [
        (
            "journal_preparations",
            "SELECT COUNT(*) FROM journal_preparations WHERE resolved_at IS NULL",
            "unresolved journal preparation",
        ),
        (
            "catalog_operations",
            "SELECT COUNT(*) FROM catalog_operations WHERE status='prepared'",
            "prepared catalog operation",
        ),
        (
            "link_key_rotations",
            "SELECT COUNT(*) FROM link_key_rotations WHERE status IN ('prepared','running')",
            "active link rotation",
        ),
        (
            "erasure_batches",
            "SELECT COUNT(*) FROM erasure_batches",
            "unfinished erasure batch",
        ),
        (
            "source_history_write_leases",
            "SELECT COUNT(*) FROM source_history_write_leases",
            "source history write lease",
        ),
        (
            "source_history_write_leases",
            "SELECT COUNT(*) FROM source_history_write_leases WHERE expires_at >= 0",
            "source history write lease",
        ),
        (
            "maintenance_jobs",
        ...
        (
            "journal_retirements",
            "SELECT COUNT(*) FROM journal_retirements WHERE retired_revision IS NULL",
            "unreconciled journal retirement",
        ),
        (
            "deletion_discovery",
            "SELECT COUNT(*) FROM deletion_discovery WHERE done=0",
            "unfinished deletion discovery",
        ),
        (
            "accounts",
    ...
    for (table, sql, label) in checks {
        ...
        if has_table(connection, table)? {
            ...
            if count > 0 {
                errors.push(format!(
                    "{count} {label}(s) must be settled before conversion"
                ));
            }
        }
    }
    if has_table(connection, "documents")? {
        ...
        if pending > 0 {
            errors.push(format!(
                "{pending} document publication preparation(s) are unresolved"
            ));
        }
    }
    if has_table(connection, "agent_objects")? {
        let count: i64 = connection.query_row(
            "SELECT COUNT(*) FROM agent_objects WHERE expires_at > ?1",
            [now_ms()],
            |row| row.get(0),
        )?;
        if count > 0 {
            errors.push(format!(
                "{count} unexpired agent object(s) must be settled before conversion"
            ));
        }
    }
    Ok(errors)
...
fn make_plan(
    ...
    for doc in all {
        if let Err(error) = validate_document_id(&doc.storage_id) {
            errors.push(error.to_string());
            continue;
        }
        if let Err(error) = validate_key(&doc.main_path) {
            errors.push(format!(
                "document {} has invalid main path: {error}",
                doc.storage_id
            ));
            continue;
        }
        if partial && !allowed.contains(doc.storage_id.as_str()) {
            exclusions.push(Exclusion {
                ...
                slug: doc.slug,
                reason: "excluded by explicit document allowlist".into(),
            });
            continue;
        }
        if doc.status == "deleting" {
            if partial {
                exclusions.push(Exclusion {
                    storage_id: doc.storage_id,
                    slug: doc.slug,
                    reason: "source document is deleting".into(),
                });
            } else {
                errors.push(format!(
                    "document {} is deleting; finish deletion or explicitly exclude it",
                    doc.storage_id
                ));
            }
            continue;
        ...
        if !matches!(
            doc.source_format.as_str(),
            "markdown" | "html" | "typst" | "latex" | "quarto"
        ) {
            errors.push(format!(
                "document {} has unsupported source format `{}`",
                doc.storage_id, doc.source_format
            ));
            continue;
        }
    ...
    for requested in allowlist {
        if !source_documents(connection)?
            .iter()
            .any(|doc| &doc.storage_id == requested)
        {
...
fn source_identity(root: &Path, connection: &Connection, catalog_digest: &str) -> Result<String> {
    ...
    if has_table(connection, "journal_state")? {
        let value: String = connection.query_row(
            "SELECT deployment_id FROM journal_state WHERE id=1",
            [],
            |row| row.get(0),
        )?;
        if !value.is_empty() {
...
fn target_identity(root: &Path, source: &str) -> Result<String> {
    ...
    if path.is_file() {
        ...
        return Err(Error::Invalid(
            "target deployment.id is not a hexadecimal identity".into(),
        ));
    }
...
fn new_manifest(
    ...
    let documents = plan
        ...
        .map(|doc| {
            (
                doc.storage_id.clone(),
                DocumentProgress {
                    slug: doc.slug.clone(),
                    ..DocumentProgress::default()
                },
            )
        })
        .collect();
    ...
    let mut file = OpenOptions::new()
        ...
        .or_else(|e| {
            if e.kind() == io::ErrorKind::AlreadyExists {
                Ok(File::open(&identity_path)?)
            } else {
                Err(e)
            }
        })?;
    if file.metadata()?.len() == 0 {
        file.write_all(target_id.as_bytes())?;
        file.write_all(b"\n")?;
        file.sync_all()?;
    }
    Ok(connection)
...
fn open_target_existing(root: &Path, manifest: &Manifest) -> Result<Connection> {
    ...
    if version != V2_USER_VERSION {
        return Err(Error::Invalid(format!(
            "resume target reports user_version {version}, expected 2"
        )));
    }
    ...
    if id != manifest.target_identity {
        return Err(Error::Invalid(
            "target identity differs from conversion manifest".into(),
        ));
    }
    let identity_path = root.join("state").join("deployment.id");
    let file_identity = fs::read_to_string(&identity_path)
        .map_err(|e| Error::Invalid(format!("target deployment identity is unreadable: {e}")))?;
    if file_identity.trim() != manifest.target_identity {
        return Err(Error::Invalid(
            "target state/deployment.id differs from conversion manifest".into(),
        ));
    }
    Ok(connection)
}

fn account_kind(provider: &str) -> &'static str {
...
fn insert_accounts(
    ...
    let account_rows: Vec<(
        ...
        let mut rows = source_accounts.query([])?;
    ...
    let has_activity = has_table(source, "account_activity")?;
    let has_preferences = has_table(source, "account_quota_preferences")?;
    for (id, provider, handle, name, email, first, last, plan, status) in account_rows {
        let (last_active,): (Option<i64>,) = if has_activity {
            (source
                .query_row(
                    "SELECT last_qualified_at FROM account_activity WHERE account_id=?1",
                    [&id],
                    |r| r.get::<_, String>(0),
                )
                .optional()?
                .map(|v| parse_time(&v, "account_activity.last_qualified_at"))
                .transpose()?,)
        } else {
            (None,)
        };
        let (preferences, preferences_revision) = if has_preferences {
            source
                .query_row(
                    "SELECT payload,revision FROM account_quota_preferences WHERE account_id=?1",
                    [&id],
                    |r| Ok((r.get::<_, String>(0)?, r.get::<_, i64>(1)?)),
                )
                .optional()?
                .unwrap_or_else(|| ("{\"version\":1}".into(), 0))
        } else {
            ("{\"version\":1}".into(), 0)
        };
        if preferences_revision < 0 {
            return Err(Error::Invalid(format!(
        ...
        let session_generation =
            deterministic_id("session-generation", &format!("{target_id}:{id}"));
        let provider_subject = if provider.is_empty() {
            None
        } else {
            Some(
    ...
    if has_table(source, "account_examples")? {
        let document_ids: HashMap<String, String> = docs
            .iter()
            .map(|doc| (doc.slug.clone(), doc.storage_id.clone()))
            .collect();
        ...
        for row in examples.query_map([], |r| {
            Ok((
                r.get::<_, String>(0)?,
                r.get::<_, i64>(1)?,
                r.get::<_, String>(2)?,
            ...
            if !(0..=4).contains(&position) || !(0..=1).contains(&completed) {
                return Err(Error::Invalid(format!(
                    "invalid account onboarding entry {account_id}/{position}"
                )));
            }
            let _ = document_id;
            onboarding
                .entry(account_id)
                .or_default()
                .push(json!({"position":position,"slug":slug,"completed":completed != 0}));
        }
...
fn document_owner(doc: &SourceDocument) -> String {
    if doc.example {
        "system".into()
    } else if let Some(id) = &doc.owner_id {
        id.clone()
    } else {
...
fn insert_documents(
    ...
    for doc in docs {
        ...
        if !title_keys.insert((owner.clone(), title_key_value.clone())) {
            return Err(Error::Invalid(format!(
                "title conflict for owner {owner}: {}",
                doc.title
            )));
        }
        let mode = if doc.example { "example" } else { "owned" };
        let status = if doc.status == "active" {
            "active"
        } else {
            "creating"
        };
        let (retention_mode, retention_revision, retention_json) = if has_table(
            source,
            ...
            if let Some((source_mode, revision, enrolled_at, last_scheduled_at)) = policy {
                let target_mode = match source_mode.as_str() {
                    "balanced" => "balanced",
                    "custom" => "manual",
                    other => {
                        return Err(Error::Invalid(format!(
                            "document {} has unsupported retention mode {other}",
                            doc.storage_id
                        )))
                    }
                };
                let payload = json_text(
                    &json!({"version":1,"source_mode":source_mode,"enrolled_at":enrolled_at,"last_scheduled_at":last_scheduled_at}),
                    16_384,
                    "retention policy",
                )?;
    ...
    for doc in docs {
        let existing: Option<String> = tx
            .query_row("SELECT id FROM documents WHERE slug=?1", [&doc.slug], |r| {
                r.get(0)
            })
            .optional()?;
        if existing.as_deref() != Some(&doc.storage_id) {
            return Err(Error::Invalid(format!(
                "target slug {} maps to a different document",
                doc.slug
            )));
        }
    }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
struct V2RecipeEnvelope {
    version: u16,
    recipe: V2Recipe,
    chunk_locators: Vec<V2Locator>,
}
#[derive(Clone, Debug, Serialize, Deserialize)]
struct V2TreeFileLocator {
    kind: String,
    file_id: String,
    logical_digest: [u8; 32],
    logical_length: u64,
    recipe: Option<V2Locator>,
    asset: Option<V2Locator>,
}
#[derive(Clone, Debug, Serialize, Deserialize)]
struct V2TreeEnvelope {
    version: u16,
    main_path: String,
    source_format: String,
...
fn digest_array(hex_digest: &str, field: &str) -> Result<[u8; 32]> {
    let bytes = hex::decode(hex_digest)
        .map_err(|e| Error::Invalid(format!("{field} is not hexadecimal: {e}")))?;
    bytes
        .try_into()
        .map_err(|_| Error::Invalid(format!("{field} is not a SHA-256 digest")))
}

fn encode_v2_source_chunks(
    doc: &SourceDocument,
    bytes: &[u8],
    file_digest: &str,
) -> Result<(V2Recipe, Vec<ObjectRecord>, Vec<V2Locator>)> {
    ...
    if bytes.len() <= 4096 {
        ...
        for chunk in
            FastCDC::with_level_and_seed(bytes, 1024, 4096, 32 * 1024, Normalization::Level1, 0)
        {
            let raw = &bytes[chunk.offset..chunk.offset + chunk.length];
            let digest: [u8; 32] = Sha256::digest(raw).into();
            parts.push((raw, digest));
        }
    }
    if parts.is_empty() {
        return Err(Error::Invalid("empty source file cannot be encoded".into()));
    }
    let codec = if bytes.len() <= 4096 {
        V2Codec::WholeZstd
    } else {
        V2Codec::ChunkedZstd
    };
    ...
    let mut locators = Vec::with_capacity(parts.len());
    let mut seen = HashSet::new();
    for (raw, digest) in parts {
        let digest_hex = hex::encode(digest);
        references.push(V2ChunkRef {
            digest,
            length: u32::try_from(raw.len())
                .map_err(|_| Error::Invalid("source chunk exceeds v2 length limit".into()))?,
        });
        let object_id = deterministic_id(
            "source-chunk",
            &format!("{}:{}", doc.storage_id, digest_hex),
        );
        let encoded = zstd::stream::encode_all(raw, 3)
            .map_err(|e| Error::Invalid(format!("source chunk could not be zstd encoded: {e}")))?;
        let encoded_digest = sha256(&encoded);
        let locator = V2Locator {
            object_id: object_id.clone(),
            object_digest: digest_array(&encoded_digest, "source chunk object digest")?,
...
fn value_string(value: &Value, field: &str) -> Result<String> {
    value
        ...
        .ok_or_else(|| Error::Invalid(format!("tree field {field} is not a string")))
}
...
fn load_tree(
    ...
    for key in candidates {
        if let Some(bytes) = read_object_optional(source_root, &key)? {
            body = Some(bytes);
            break;
        }
    }
    let Some(body) = body else {
        ...
        return Ok(SourceTree {
            main: doc.main_path.clone(),
            files: vec![SourceFile {
                path: doc.main_path.clone(),
                kind: "text".into(),
                id: deterministic_id("legacy-file", &format!("{}:{tree_sha}", doc.storage_id)),
                sha: sha256(&bytes),
    ...
    if tree_sha.len() == 64 && sha256(&body) != tree_sha {
        return Err(Error::Invalid(format!(
            "checkpoint tree {tree_sha} failed byte digest verification"
        )));
    }
    let value: Value = serde_json::from_slice(&body).map_err(|e| {
        Error::Invalid(format!("checkpoint {} tree is invalid JSON: {e}", tree_sha))
    })?;
    let main = value
        .get("main")
        .and_then(Value::as_str)
        .unwrap_or(&doc.main_path)
        .to_string();
    ...
    let object = value
        ...
        .ok_or_else(|| {
            Error::Invalid(format!("checkpoint {} tree has no files object", tree_sha))
        })?;
    ...
    for (path, raw) in object {
        ...
        let sha = raw
            ...
            .and_then(Value::as_str)
            .ok_or_else(|| {
                Error::Invalid(format!("checkpoint {} file {path} has no digest", tree_sha))
            })?
            .to_string();
        let size = raw.get("size").and_then(Value::as_i64).unwrap_or(-1);
        if size < 0 {
            return Err(Error::Invalid(format!(
                "checkpoint {} file {path} has invalid size",
                tree_sha
            )));
        }
...
fn decode_recipe(bytes: &[u8]) -> Result<RecipeInfo> {
    ...
    if cursor.take(8)? != b"LPREC001" {
        return Err(Error::Invalid("unsupported source recipe format".into()));
    }
    let version = cursor.u16()?;
    let _profile = cursor.u16()?;
    let codec = cursor.u8()?;
    if cursor.u8()? != 0 {
        return Err(Error::Invalid(
            "source recipe reserved flags are set".into(),
        ));
    }
    let length = cursor.u64()?;
    let file_digest = hex::encode(cursor.take(32)?);
    let count = cursor.u32()? as usize;
    let expected = 58usize
        .checked_add(
            count
                .checked_mul(36)
                .ok_or_else(|| Error::Invalid("source recipe size overflow".into()))?,
        )
        .ok_or_else(|| Error::Invalid("source recipe size overflow".into()))?;
    if version != 1 || count > 1_000_000 || (codec != 1 && codec != 2) || bytes.len() != expected {
        return Err(Error::Invalid("invalid source recipe header".into()));
    }
    let mut chunks = Vec::with_capacity(count);
    let mut total = 0u64;
    for _ in 0..count {
        let digest = hex::encode(cursor.take(32)?);
        let len = cursor.u32()?;
        total = total
            .checked_add(len as u64)
            .ok_or_else(|| Error::Invalid("source recipe length overflow".into()))?;
        chunks.push((digest, len));
    }
    if total != length || (codec == 1 && count != 1) {
        return Err(Error::Invalid(
            "source recipe lengths are inconsistent".into(),
        ));
    }
    Ok(RecipeInfo {
        file_digest,
...
fn source_history_file(
    source_root: &Path,
    connection: &Connection,
    doc: &SourceDocument,
    digest: &str,
) -> Result<Option<Vec<u8>>> {
    if !has_table(connection, "source_history_encodings")? {
        return Ok(None);
    }
    ...
    let Some((recipe_key, recipe_digest, codec, uncompressed_bytes, recipe_byte_count)) = row
    else {
        return Ok(None);
    };
    if codec <= 0 || uncompressed_bytes < 0 || recipe_byte_count < 0 {
        return Err(Error::Invalid(format!(
            "source history encoding for {digest} has invalid accounting"
        )));
    }
    let recipe_bytes = read_object(source_root, &recipe_key)?;
    if recipe_bytes.len() as i64 != recipe_byte_count || sha256(&recipe_bytes) != recipe_digest {
        return Err(Error::Invalid(format!(
            "source history recipe {recipe_key} failed descriptor integrity"
        )));
    }
    let recipe = decode_recipe(&recipe_bytes)?;
    if recipe.file_digest != digest || recipe.uncompressed_len as i64 != uncompressed_bytes {
        return Err(Error::Invalid(format!(
            "source recipe for {digest} has inconsistent metadata"
        )));
    }
    ...
    if has_table(connection, "source_history_objects")? {
        ...
        for row in st.query_map(params![doc.storage_id, digest], |r| {
            Ok((
                r.get::<_, String>(0)?,
                r.get::<_, String>(1)?,
                r.get::<_, i64>(2)?,
            ...
            let (key, kind, accounted) = row?;
            if kind != "source_chunk" || accounted < 0 {
                return Err(Error::Invalid(format!(
                    "source history object {key} has unsupported kind or size"
                )));
            }
            let bytes = read_object(source_root, &key)?;
            if accounted > 0 && accounted != bytes.len() as i64 {
                return Err(Error::Invalid(format!(
                    "source history object {key} has incorrect byte accounting"
                )));
            }
    ...
    for (chunk_digest, expected_len) in &recipe.chunks {
        ...
        for (key, encoded) in &encoded_by_digest {
            let chunk = zstd::stream::decode_all(encoded.as_slice()).map_err(|e| {
                Error::Invalid(format!("source history chunk {key} cannot be decoded: {e}"))
            })?;
            if sha256(&chunk) == *chunk_digest && chunk.len() == *expected_len as usize {
                matches.push(chunk);
            }
        }
        if matches.len() != 1 {
            return Err(Error::Invalid(format!("source recipe {digest} resolves to {} objects for chunk {chunk_digest}; refusing ambiguous or missing history",matches.len())));
        }
        out.extend_from_slice(&matches[0]);
    }
    if out.len() as u64 != recipe.uncompressed_len || sha256(&out) != digest {
        return Err(Error::Invalid(format!(
            "source history file {digest} failed reconstruction"
        )));
    }
    Ok(Some(out))
}
...
fn read_file_bytes(
    ...
    if kind != "asset" {
        if let Some(connection) = history {
            if let Some(bytes) = source_history_file(source_root, connection, doc, digest)? {
                return Ok(bytes);
            }
        }
    }
    let candidates = if kind == "asset" {
        vec![
            format!("content/{}/assets/{digest}", doc.storage_id),
            format!("content/{}/assets/{digest}", doc.slug),
        ]
    } else {
        vec![
            format!("content/{}/blobs/{digest}", doc.storage_id),
            format!("content/{}/blobs/{digest}", doc.slug),
            format!("sources/{}/{}", doc.slug, digest),
            format!("history/{}/{digest}", doc.slug),
        ]
    };
    for key in candidates {
        if let Some(bytes) = read_object_optional(source_root, &key)? {
    ...
    Err(Error::Invalid(format!(
        "source {kind} bytes {digest} for {} are missing or have the wrong digest",
        doc.storage_id
    )))
}

fn source_checkpoint_rows(
    connection: &Connection,
    doc: &SourceDocument,
    ...
    let mut rows = statement.query([&doc.slug])?;
    let mut out = Vec::new();
    while let Some(row) = rows.next()? {
        let at: String = row.get(4)?;
        let parent: String = row.get(3)?;
        let changed: Option<String> = row.get(12)?;
        let changed = changed
            .map(|v| serde_json::from_str(&v))
            .transpose()?
            .unwrap_or_default();
        out.push(SourceCheckpoint {
            id: row.get(0)?,
            seq: row.get(1)?,
            tree_sha: row.get(2)?,
            ...
            at: parse_time(&at, "checkpoints.at")?,
            by: row.get(5)?,
            by_account: row.get(13)?,
            why: row.get(6)?,
            source_format: {
                let v: String = row.get(7)?;
                if v.is_empty() {
                    doc.source_format.clone()
                } else {
                    v
                }
            },
        ...
        if out.len() > MAX_CHECKPOINTS {
            return Err(Error::Invalid(format!(
                "document {} exceeds {} checkpoint limit",
                doc.storage_id, MAX_CHECKPOINTS
            )));
        }
    }
    Ok(out)
...
fn object_record(
    ...
    ObjectRecord {
        key: format!("v2/documents/{}/objects/{id}", doc.storage_id),
        digest: sha256(&bytes),
        id,
        kind: kind.into(),
        bytes,
...
fn convert_checkpoint(
    ...
    if has_table(source, "checkpoint_asset_refs")? {
        ...
        for row in refs.query_map(params![doc.storage_id, point.id], |r| {
            ...
            validate_key(&key)?;
            let bytes = read_object(source_root, &key)?;
            if declared_bytes < 0 || bytes.len() as i64 != declared_bytes {
                return Err(Error::Invalid(format!(
                    "checkpoint {} asset reference {} has wrong length",
                    point.id, key
                )));
            }
    ...
    if has_table(source, "checkpoint_asset_sets")? {
        ...
        if let Some(count) = declared {
            ...
            if count != actual {
                return Err(Error::Invalid(format!(
                    "checkpoint {} asset set count {count} does not match tree count {actual}",
                    point.id
                )));
            }
        }
    ...
    let mut logical_files = BTreeMap::new();
    ...
    for file in tree.files {
        ...
        if bytes.len() > MAX_OBJECT_BYTES {
            return Err(Error::Invalid(format!(
                "checkpoint {} file {} exceeds {} byte limit",
                point.id, file.path, MAX_OBJECT_BYTES
            )));
        }
        if bytes.len() as i64 != file.size || sha256(&bytes) != file.sha {
            return Err(Error::Invalid(format!(
                "checkpoint {} file {} failed byte verification",
                point.id, file.path
            )));
        }
        logical_bytes = logical_bytes
            .checked_add(file.size)
            .ok_or_else(|| Error::Invalid("checkpoint logical byte overflow".into()))?;
        ...
        if kind == "asset" {
            ...
            let locator = V2Locator {
                object_id: object_id.clone(),
                object_digest: digest_array(&object_digest, "asset object digest")?,
                logical_digest: Some(logical_digest),
                logical_length: file.size as u64,
                byte_length: file.size as u64,
                encoding_version: 1,
            };
            tree_files.insert(
                file.path.clone(),
            ...
            let recipe_id =
                deterministic_id("source-recipe", &format!("{}:{}", doc.storage_id, file.sha));
            let (recipe, chunk_records, chunk_locators) =
                encode_v2_source_chunks(doc, &bytes, &file.sha)?;
            let recipe_wire = V2RecipeEnvelope {
                version: 1,
                recipe,
                chunk_locators,
            };
            ...
            for chunk in chunk_records {
                object_ids.insert(chunk.id.clone());
                records.push(chunk);
            }
            object_ids.insert(recipe_id.clone());
            records.push(recipe);
            let recipe_locator = V2Locator {
                object_id: recipe_id,
                object_digest: digest_array(&sha256(&recipe_body), "source recipe object digest")?,
                logical_digest: Some(logical_digest),
                logical_length: file.size as u64,
    ...
    if tree_files.is_empty() || !tree_files.contains_key(&tree.main) {
        return Err(Error::Invalid(format!(
            "checkpoint {} tree has no main file",
            point.id
        )));
    }
    let settings_json = match tree.settings {
        Some(value) => json_text(&value, 16 * 1024 * 1024, "tree settings")?,
        None => "{\"version\":1}".into(),
    };
    let logical_settings = serde_json::from_str::<Value>(&settings_json)
        .ok()
        .and_then(|value| {
            value
                .get("engine")
        ...
        .map(|engine| LogicalSettings { engine });
    let logical_body = serde_json::to_vec(&LogicalTree {
        main: tree.main.clone(),
        files: logical_files,
        settings: logical_settings,
    })?;
    let logical_digest: [u8; 32] = Sha256::digest(&logical_body).into();
    let tree_envelope = V2TreeEnvelope {
        version: 1,
        main_path: tree.main,
    ...
    let tree_id = deterministic_id(
        "source-tree",
        &format!("{}:{}", doc.storage_id, point.tree_sha),
    );
    let tree_record = object_record(
        doc,
        tree_id.clone(),
        "source_tree",
        tree_bytes,
        Some(hex::encode(logical_digest)),
    );
    let tree_digest = hex::encode(logical_digest);
    object_ids.insert(tree_id.clone());
...
impl<'a> Cursor<'a> {
    ...
    fn take(&mut self, n: usize) -> Result<&'a [u8]> {
        let end = self
            .offset
            .checked_add(n)
            .ok_or_else(|| Error::Invalid("journal length overflow".into()))?;
        if end > self.bytes.len() {
            return Err(Error::Invalid("truncated journal object".into()));
        }
    ...
    fn u16(&mut self) -> Result<u16> {
        Ok(u16::from_le_bytes(
            self.take(2)?
                .try_into()
                .map_err(|_| Error::Invalid("journal u16".into()))?,
        ))
    }
    fn u32(&mut self) -> Result<u32> {
        Ok(u32::from_le_bytes(
            self.take(4)?
                .try_into()
                .map_err(|_| Error::Invalid("journal u32".into()))?,
        ))
    }
    fn u64(&mut self) -> Result<u64> {
        Ok(u64::from_le_bytes(
            self.take(8)?
                .try_into()
                .map_err(|_| Error::Invalid("journal u64".into()))?,
        ))
    }
    fn text(&mut self) -> Result<String> {
        let n = self.u16()? as usize;
        String::from_utf8(self.take(n)?.to_vec())
            .map_err(|_| Error::Invalid("journal text is not UTF-8".into()))
    }
}

fn decode_segment(bytes: &[u8]) -> Result<Vec<JournalRecord>> {
    let mut c = Cursor::new(bytes);
    if c.take(4)? != b"KJNL" || c.u16()? != 2 {
        return Err(Error::Invalid("unsupported journal segment format".into()));
    }
    let count = c.u32()? as usize;
    if count == 0 || count > 4096 {
        return Err(Error::Invalid("invalid journal record count".into()));
    }
    let mut records = Vec::with_capacity(count);
    for _ in 0..count {
        if c.u16()? != 2 {
            return Err(Error::Invalid("journal record format mismatch".into()));
        }
        let storage_id = c.text()?;
        let sequence = c.u64()?;
        let epoch = c.u64()?;
        let fragment_index = c.u32()?;
        let fragment_count = c.u32()?;
        let retry = c.text()?;
        let digest = c.text()?;
        let chunk_digest = c.text()?;
        ...
        if storage_id.is_empty()
            ...
            || n > 4 * 1024 * 1024
        {
            return Err(Error::Invalid("invalid journal fragment".into()));
        }
        let payload = c.take(n)?.to_vec();
        if sha256(&payload) != chunk_digest {
            return Err(Error::Invalid(format!(
                "journal chunk digest mismatch at sequence {sequence}"
            )));
        }
        if fragment_count == 1 && sha256(&payload) != digest {
            return Err(Error::Invalid(format!(
                "journal record digest mismatch at sequence {sequence}"
            )));
        }
    ...
    if c.offset != bytes.len() {
        return Err(Error::Invalid("journal segment has trailing bytes".into()));
    }
    Ok(records)
}
...
fn decode_base(bytes: &[u8]) -> Result<(String, u64, u64, Vec<u8>)> {
    let mut c = Cursor::new(bytes);
    if c.take(4)? != b"KJBS" || c.u16()? != 1 {
        return Err(Error::Invalid("unsupported journal base format".into()));
    }
    if c.u16()? != 2 {
        return Err(Error::Invalid(
            "unsupported journal base segment format".into(),
        ));
    }
    let storage = c.text()?;
    let epoch = c.u64()?;
    ...
    if c.offset != bytes.len()
        || storage.is_empty()
        || sequence == 0
        || payload.len() > MAX_OBJECT_BYTES
        || digest != sha256(&payload)
        || payload.is_empty()
    {
        return Err(Error::Invalid("invalid journal base integrity".into()));
    }
    Ok((storage, epoch, sequence, payload))
}

fn encode_recovery_base(
    storage_id: &str,
    epoch: u64,
    sequence: u64,
    payload: &[u8],
    ...
    if storage_id.is_empty()
        || storage_id.len() > u16::MAX as usize
        || sequence == 0
        || payload.is_empty()
        || payload.len() > MAX_OBJECT_BYTES
    {
        return Err(Error::Invalid(
            "invalid reconstructed journal base identity or size".into(),
        ));
    }
    let digest = sha256(payload);
    let mut out = Vec::with_capacity(64 + storage_id.len() + digest.len() + payload.len());
    out.extend_from_slice(b"KJBS");
    out.extend_from_slice(&1u16.to_le_bytes());
    out.extend_from_slice(&2u16.to_le_bytes());
    out.extend_from_slice(&(storage_id.len() as u16).to_le_bytes());
    out.extend_from_slice(storage_id.as_bytes());
...
#[derive(Deserialize, Serialize)]
struct ManifestShardDescriptor {
    shard_id: String,
    shard_seq: u64,
    object_key: String,
    digest: String,
...
fn journal_manifest_descriptors(
    source_root: &Path,
    source: &Connection,
    doc: &SourceDocument,
) -> Result<Option<(Vec<ManifestBaseDescriptor>, Vec<String>)>> {
    if !has_table(source, "journal_state")? {
        return Ok(None);
    }
    ...
    let Some((root, root_digest, root_len, _revision)) = state else {
        return Ok(None);
    };
    if root.is_empty() {
        return Ok(None);
    ...
    for index in 0..4096 {
        if !visited.insert(key.clone()) {
            return Err(Error::Invalid("journal manifest shard cycle".into()));
        }
        let bytes = read_object(source_root, &key)?;
        let shard: ManifestShardDescriptor = serde_json::from_slice(&bytes)
            .map_err(|e| Error::Invalid(format!("journal manifest shard {key} is invalid: {e}")))?;
        if shard.object_key != key || shard.encoded_bytes != bytes.len() as i64 {
            return Err(Error::Invalid(format!(
                "journal manifest shard {key} identity/length mismatch"
            )));
        }
        ...
        if sha256(&canonical_bytes) != digest {
            return Err(Error::Invalid(format!(
                "journal manifest shard {key} digest mismatch"
            )));
        }
        if index == 0 && (digest != root_digest || bytes.len() as i64 != root_len) {
            return Err(Error::Invalid(
                "journal manifest root differs from journal_state".into(),
            ));
        }
        let next = canonical.next_key.clone();
        bases.extend(
            canonical
                .bases
                .into_iter()
                .filter(|base| base.storage_id == doc.storage_id),
        );
        segments.extend(canonical.segments);
        match next {
            Some(next) => key = next,
            None => break,
        };
        if index == 4095 {
            return Err(Error::Invalid("journal manifest chain is too long".into()));
        }
    }
...
fn journal_replay(
    ...
    if let Some((bases, _segments)) = &manifest_descriptors {
        if let Some(base) = bases.iter().max_by_key(|base| (base.epoch, base.sequence)) {
            let bytes = read_object(source_root, &base.object_key)?;
            if bytes.len() as i64 != base.encoded_bytes || sha256(&bytes) != base.digest {
                return Err(Error::Invalid(format!(
                    "journal base {} failed descriptor integrity",
                    base.object_key
                )));
            }
            let (storage, epoch, sequence, payload) = decode_base(&bytes)?;
            if storage != doc.storage_id || epoch != base.epoch || sequence != base.sequence {
                return Err(Error::Invalid(format!(
                    "journal base {} identity mismatch",
                    base.object_key
                )));
            }
            latest = Some((epoch, sequence, payload));
        }
        ...
        for row in st.query_map([&doc.storage_id], |r| {
            Ok((
                r.get::<_, String>(0)?,
                r.get::<_, String>(1)?,
                r.get::<_, i64>(2)?,
            ...
            let (key, digest, epoch, sequence) = row?;
            let bytes = read_object(source_root, &key)?;
            if sha256(&bytes) != digest {
                return Err(Error::Invalid(format!(
                    "journal base {key} digest mismatch"
                )));
            }
            let (storage, body_epoch, body_seq, payload) = decode_base(&bytes)?;
            if storage != doc.storage_id
                || body_epoch as i64 != epoch
                || body_seq as i64 != sequence
            {
                return Err(Error::Invalid(format!(
                    "journal base {key} identity mismatch"
                )));
            }
            latest = Some((body_epoch, body_seq, payload));
            break;
        }
    }
    let base_identity = latest.as_ref().map(|(e, s, _)| (*e, *s));
    let mut fragments: BTreeMap<(u64, u64), Vec<JournalRecord>> = BTreeMap::new();
    if let Some((_bases, segment_keys)) = &manifest_descriptors {
        for key in segment_keys {
            let bytes = read_object(source_root, key)?;
            for record in decode_segment(&bytes)?
                .into_iter()
                .filter(|r| r.storage_id == doc.storage_id)
            {
                fragments
                    .entry((record.epoch, record.sequence))
                    .or_default()
                    .push(record);
            }
        }
        ...
        let mut st = source
            .prepare("SELECT object_key,digest FROM journal_segments ORDER BY segment_seq")?;
        for row in st.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))? {
            let (key, digest) = row?;
            let bytes = read_object(source_root, &key)?;
            if sha256(&bytes) != digest {
                return Err(Error::Invalid(format!(
                    "journal segment {key} digest mismatch"
                )));
            }
            for record in decode_segment(&bytes)?
                .into_iter()
    ...
    for ((epoch, sequence), parts) in fragments {
        if base_identity.is_some_and(|base| (epoch, sequence) <= base) {
            continue;
        }
        let first = parts
            .first()
            .ok_or_else(|| Error::Invalid("empty journal fragment set".into()))?;
        let expected_fragments = first.fragment_count;
        let expected_digest = first.digest.clone();
        if parts.len() != expected_fragments as usize
            || parts
                .iter()
                .any(|p| p.fragment_count != expected_fragments || p.digest != expected_digest)
        {
            return Err(Error::Invalid(format!(
                "journal sequence {sequence} has incomplete fragments"
            )));
        }
        let mut ordered = parts;
        ordered.sort_by_key(|p| p.fragment_index);
        if ordered
            .iter()
            .enumerate()
            .any(|(i, p)| p.fragment_index as usize != i)
        {
            return Err(Error::Invalid(format!(
                "journal sequence {sequence} has a fragment gap"
            )));
        }
        let total = ordered
            .iter()
            .try_fold(0usize, |sum, part| sum.checked_add(part.payload.len()))
            .ok_or_else(|| Error::Invalid("journal update length overflow".into()))?;
        if total > MAX_OBJECT_BYTES {
            return Err(Error::Invalid(format!(
                "journal sequence {sequence} exceeds object size limit"
            )));
        }
        let payload = ordered
            .into_iter()
        ...
        if sha256(&payload) != expected_digest {
            return Err(Error::Invalid(format!(
                "journal sequence {sequence} digest mismatch"
            )));
        }
    ...
    if latest.is_none()
        && updates
            .keys()
            .next()
            .is_some_and(|(epoch, sequence)| *epoch != 0 || *sequence != 1)
    {
        return Err(Error::Invalid(format!(
            "journal for {} has no base covering its first sequence",
            doc.storage_id
        )));
    }
    if latest.is_none() && updates.is_empty() {
        let session = read_object_optional(source_root, &format!("sessions/{}", doc.slug))?;
        let Some(session) = session.filter(|bytes| !bytes.is_empty()) else {
            return Ok(None);
        };
        let ydoc = Doc::new();
        let update = Update::decode_v1(&session)
            .map_err(|e| Error::Invalid(format!("live session CRDT is invalid: {e}")))?;
        ydoc.transact_mut()
            .apply_update(update)
            .map_err(|e| Error::Invalid(format!("live session cannot be applied: {e}")))?;
        let payload = ydoc
            .transact()
            .encode_state_as_update_v1(&yrs::StateVector::default());
        return Ok(Some(ReplayedJournal {
            payload,
            epoch: 0,
            sequence: 1,
        }));
    ...
    let ydoc = Doc::new();
    if let Some((_, _, payload)) = &latest {
        let update = Update::decode_v1(payload)
            .map_err(|e| Error::Invalid(format!("journal base CRDT is invalid: {e}")))?;
        ydoc.transact_mut()
            .apply_update(update)
            .map_err(|e| Error::Invalid(format!("journal base cannot be applied: {e}")))?;
    }
    let mut expected = latest
        .as_ref()
        .map_or((0, 0), |(epoch, sequence, _)| (*epoch, *sequence));
    let mut last = expected;
    for ((epoch, sequence), payload) in updates {
        if (epoch, sequence) < expected {
            continue;
        }
        if epoch == expected.0 {
            if sequence != expected.1.saturating_add(1) {
                return Err(Error::Invalid(format!(
                    "journal sequence coverage gap before epoch {epoch} sequence {sequence}"
                )));
            }
        } else if epoch > expected.0 && sequence != 1 {
            return Err(Error::Invalid(format!(
                "journal epoch {epoch} starts at sequence {sequence}, expected 1"
            )));
        }
        let update = Update::decode_v1(&payload).map_err(|e| {
            Error::Invalid(format!("journal update {epoch}/{sequence} is invalid: {e}"))
        })?;
        ydoc.transact_mut().apply_update(update).map_err(|e| {
            Error::Invalid(format!(
                "journal update {epoch}/{sequence} cannot be applied: {e}"
            ))
        })?;
        expected = (epoch, sequence);
        last = expected;
    }
    if latest.is_none() && last == (0, 0) {
        return Err(Error::Invalid(
            "journal contained no applicable updates".into(),
        ));
    }
    let payload = ydoc
        .transact()
...
fn insert_object(
    ...
    let existing: (String, i64) = tx.query_row(
        "SELECT digest,byte_length FROM objects WHERE document_id=?1 AND id=?2",
        params![record.key.split('/').nth(2).unwrap_or_default(), record.id],
        |r| Ok((r.get(0)?, r.get(1)?)),
    )?;
    if existing.0 != record.digest || existing.1 != record.bytes.len() as i64 {
        return Err(Error::Invalid(format!(
            "target object {} identity differs on resume",
            record.id
        )));
    }
    Ok(())
...
fn target_account_exists(tx: &Transaction<'_>, account: Option<&str>) -> Result<Option<String>> {
    ...
    Ok(tx
        .query_row("SELECT id FROM accounts WHERE id=?1", [account], |r| {
            r.get(0)
        })
        .optional()?)
}
...
fn convert_document(
    ...
    target: &mut Connection,
    doc: &SourceDocument,
    progress: &mut DocumentProgress,
) -> Result<()> {
    let points = source_checkpoint_rows(source, doc)?;
    let journal = journal_replay(source_root, source, doc)?;
    let mut converted = Vec::new();
    let mut all_objects: BTreeMap<String, ObjectRecord> = BTreeMap::new();
    for point in points {
        let converted_point = convert_checkpoint(source_root, source, doc, point)?;
        for record in &converted_point.objects {
            write_object(target_root, &record.key, &record.bytes)?;
            all_objects
    ...
    let journal_record = if let Some(state) = journal.as_ref() {
        let encoded =
            encode_recovery_base(&doc.storage_id, state.epoch, state.sequence, &state.payload)?;
        let mut record = object_record(
            doc,
            deterministic_id("journal-base", &doc.storage_id),
            "journal_base",
            encoded,
            None,
        );
    ...
    let tx = target.transaction()?;
    for record in all_objects.values() {
        insert_object(&tx, record, record.journal)?;
    }
    if let Some(record) = &journal_record {
        if let Some(state) = &journal {
            tx.execute("UPDATE objects SET journal_epoch=?3,first_sequence=?4,last_sequence=?5,live_root=1 WHERE document_id=?1 AND id=?2",params![doc.storage_id,record.id,state.epoch as i64,state.sequence as i64,state.sequence as i64])?;
        }
    }
    let mut closure_count = 0u64;
    let mut checkpoint_ids = HashSet::new();
    for item in &converted {
        if !checkpoint_ids.insert(item.point.id.clone()) {
            return Err(Error::Invalid(format!(
        ...
        let metadata = json_text(
            &json!({"version":1,"git_commit":item.point.commit,"dirty":item.point.dirty,"changed":item.point.changed,"original_parent":item.point.parent}),
            65536,
            "checkpoint metadata",
        )?;
        ...
        for object_id in &item.object_ids {
            closure_count = closure_count
                .checked_add(1)
                .ok_or_else(|| Error::Invalid("checkpoint closure count overflow".into()))?;
            if closure_count > 1_048_576 {
                return Err(Error::Invalid(format!(
                    "document {} exceeds checkpoint closure limit",
                    doc.storage_id
                )));
            }
    ...
    let current = converted.last();
    let current_id = current.map(|v| v.point.id.clone());
    if let Some(current) = current {
        for object_id in &current.object_ids {
            tx.execute(
                "UPDATE objects SET live_root=1 WHERE document_id=?1 AND id=?2",
                params![doc.storage_id, object_id],
            )?;
        }
    }
    let (base_id, epoch, sequence) = journal
        .as_ref()
        .map(|state| {
            (
                deterministic_id("journal-base", &doc.storage_id),
    ...
    let mapping = converted
        .iter()
        .flat_map(|item| {
            std::iter::once(format!("checkpoint:{}:{}", item.point.id, item.tree_id))
                .chain(item.object_ids.iter().map(|id| format!("object:{id}")))
        })
        .chain(std::iter::once(format!(
            "journal:{}",
            journal
                .as_ref()
                .map(|state| format!("{}:{}", state.epoch, state.sequence))
                .unwrap_or_default()
        )))
        .collect::<Vec<_>>()
        .join("\n");
    if all_objects.len() > 1_048_576 {
        return Err(Error::Invalid(format!(
            "document {} exceeds object inventory limit",
            doc.storage_id
        )));
    }
    progress.cursor = "content-and-checkpoints".into();
    progress.checkpoints = converted.len() as u64;
    progress.objects = all_objects.len() as u64;
    progress.bytes = all_objects.values().map(|o| o.bytes.len() as u64).sum();
    progress.mapping_digest = sha256(mapping.as_bytes());
    Ok(())
}
...
fn import_sharing(
    ...
    let doc_map: HashMap<String, String> = docs
        .iter()
        .map(|d| (d.slug.clone(), d.storage_id.clone()))
        .collect();
    let tx = target.transaction()?;
    ...
    let mut st =
        source.prepare("SELECT slug,account_id,role,since FROM grants ORDER BY slug,account_id")?;
    for row in st.query_map([], |r| {
        ...
        let (slug, account, role, since) = row?;
        let Some(doc_id) = doc_map.get(&slug) else {
            continue;
        };
        let effective = if role == "owner" {
            "editor"
        } else {
            role.as_str()
        };
        if !matches!(effective, "reader" | "commenter" | "editor") {
            continue;
        }
        let key = (doc_id.clone(), account.clone());
        let candidate = (effective.to_string(), parse_time(&since, "grants.since")?);
        if grants
            .get(&key)
            .is_none_or(|old| rank(&candidate.0) > rank(&old.0))
        {
            grants.insert(key, candidate);
        }
    }
    for ((doc_id, account), (role, created)) in grants {
        if target_account_exists(&tx, Some(&account))?.is_some() {
            tx.execute("INSERT OR IGNORE INTO grants(document_id,account_id,role,created_at) VALUES(?1,?2,?3,?4)",params![doc_id,account,role,created])?;
        }
    }
    ...
    for row in st.query_map([], |r| {
        ...
        let (slug, role, hash, sealed, label, budget, since, until, key_id) = row?;
        let Some(doc_id) = doc_map.get(&slug) else {
            continue;
        };
        if !matches!(role.as_str(), "reader" | "commenter" | "editor")
            || hash.len() != 64
            || !hash.bytes().all(|b| b.is_ascii_hexdigit())
        {
            return Err(Error::Invalid(format!("invalid v1 link {slug}/{role}")));
        }
        let link_id = deterministic_id("link", &format!("{doc_id}:{role}"));
        let created = parse_time(&since, "links.since")?;
        let expiry = if until.trim().is_empty() {
            None
        } else {
            Some(parse_time(&until, "links.until")?)
        };
    ...
    if has_table(source, "guests")? {
        let mut bookmarks: HashMap<String, Vec<Value>> = HashMap::new();
        let mut st = source.prepare(
            "SELECT slug,account_id,since,link_hash FROM guests ORDER BY account_id,slug,link_hash",
        )?;
        for row in st.query_map([], |r| {
            Ok((
                r.get::<_, String>(0)?,
            ...
            let (slug, account, since, hash) = row?;
            let Some(doc_id) = doc_map.get(&slug) else {
                continue;
            };
            let Some((link_id, _key, generation)) = links.get(&(slug.clone(), hash.clone())) else {
                continue;
            };
            if target_account_exists(&tx, Some(&account))?.is_none() {
                continue;
            };
            bookmarks.entry(account).or_default().push(json!({"document_id":doc_id,"link_id":link_id,"credential_generation":generation,"pinned_at":parse_time(&since,"guests.since")?}));
        }
        for (account, items) in bookmarks {
            let payload = json_text(&json!({"version":1,"items":items}), 262144, "bookmarks")?;
            tx.execute(
                "UPDATE accounts SET bookmarks_json=?2 WHERE id=?1",
...
fn import_annotations(
    ...
    target: &mut Connection,
    docs: &[SourceDocument],
    progress: &mut HashMap<String, DocumentProgress>,
) -> Result<()> {
    let doc_map: HashMap<String, String> = docs
        .iter()
        .map(|d| (d.slug.clone(), d.storage_id.clone()))
    ...
    {
        let mut stcp = tx.prepare("SELECT document_id,id FROM checkpoints")?;
        for row in stcp.query_map([], |r| Ok((r.get::<_, String>(0)?, r.get::<_, String>(1)?)))? {
            let (d, id) = row?;
            checkpoint_map.insert((d, id.clone()), id);
        }
    }
    ...
    for row in st.query_map([], |r| {
        ...
        if !annotations.insert((doc_id.clone(), id.clone())) {
            return Err(Error::Invalid(format!("duplicate annotation {slug}/{id}")));
        }
        if position.is_some_and(|value| value < 0) || source_position.is_some_and(|value| value < 0)
        {
            return Err(Error::Invalid(format!(
                "annotation {doc_id}/{id} has a negative selector position"
            )));
        }
        let region_value = region
            ...
            .map(|raw| {
                serde_json::from_str::<Value>(raw).map_err(|e| {
                    Error::Invalid(format!(
                        "annotation {doc_id}/{id} has an invalid region selector: {e}"
                    ))
                })
            })
            .transpose()?;
        ...
        let kind = match motivation.as_str() {
            "highlight" => "highlight",
        ...
        let selector = json!({"version":1,"rendered":{"kind":if region_value.is_some(){"figure_region"}else if point != 0{"point"}else if position.is_some(){"text_position"}else{"text_quote"},"exact":exact,"prefix":prefix,"suffix":suffix,"position":position,"region":region_value},"source":source_path.map(|path|json!({"path":path,"exact":source_exact,"prefix":source_prefix,"suffix":source_suffix,"position":source_position}))});
        let selector_json = json_text(&selector, 65536, "annotation selector")?;
        let context = json_text(
            &json!({"version":1,"review_pass":pass,"point":point,"color":color,"quarto_output":quarto_output}),
            16384,
            "annotation context",
        )?;
        let created_ms = parse_time(&created, "comments.created")?;
        let resolved_ms = resolved_at
            .as_deref()
            .map(|v| parse_time(v, "comments.resolved_at"))
            .transpose()?;
        let state: String = if kind == "suggestion" {
        ...
        let acceptance = if accepted {
            if accept_request.is_empty() {
                deterministic_id("legacy-accept", &format!("{doc_id}:{id}"))
            } else {
        ...
        let effective_resolved = if accepted {
            resolved_ms.or(Some(created_ms))
        } else {
            resolved_ms
        };
    ...
    for row in sr.query_map([], |r| {
        ...
        let (slug, comment, id, body, creator, author, created) = row?;
        let Some(doc_id) = doc_map.get(&slug) else {
            continue;
        };
        let author_id = target_account_exists(&tx, Some(&author))?;
    ...
    for doc in docs {
        let max: i64 = tx.query_row(
            "SELECT COALESCE(MAX(seq),0)+1 FROM annotations WHERE document_id=?1",
            [&doc.storage_id],
            |r| r.get(0),
        )?;
        tx.execute(
            "UPDATE documents SET next_annotation_seq=?2 WHERE id=?1",
            params![doc.storage_id, max],
        )?;
    }
    tx.commit()?;
    Ok(())
...
#[derive(Deserialize)]
struct V1PublicationObject {
    sha256: String,
    bytes: usize,
    mime: String,
}
#[derive(Deserialize)]
struct V1PublicationAsset {
    path: String,
    #[serde(flatten)]
    object: V1PublicationObject,
}
#[derive(Deserialize)]
struct V1Publication {
    ...
    html: V1PublicationObject,
    assets: Vec<V1PublicationAsset>,
}
...
fn convert_publication(
    ...
    let Some(manifest_bytes) = read_object_optional(
        source_root,
        &format!("publications/{}/current.json", doc.storage_id),
    )?
    else {
        if !doc.last_publication_id.is_empty() {
            return Err(Error::Invalid(format!(
                "document {} names publication {} but current manifest is missing",
                doc.storage_id, doc.last_publication_id
            )));
        }
        return Ok(());
    };
    let publication: V1Publication = serde_json::from_slice(&manifest_bytes).map_err(|e| {
        Error::Invalid(format!(
            "publication manifest {} is invalid: {e}",
            doc.storage_id
        ))
    })?;
    if publication.publication_id.is_empty() {
        return Err(Error::Invalid("publication ID is empty".into()));
    }
    let mut records = Vec::new();
    let mut record_ids = HashSet::new();
    let html = read_object(
        source_root,
        &format!(
            "publications/{}/objects/{}",
            doc.storage_id, publication.html.sha256
        ),
    )?;
    if html.len() > MAX_OBJECT_BYTES {
        return Err(Error::Invalid(
            "publication HTML exceeds object size limit".into(),
        ));
    }
    if html.len() != publication.html.bytes || sha256(&html) != publication.html.sha256 {
        return Err(Error::Invalid(format!(
            "publication HTML {} failed integrity",
            publication.html.sha256
        )));
    }
    let html_id = deterministic_id(
        "publication-html",
        &format!("{}:{}", doc.storage_id, publication.html.sha256),
    );
    if record_ids.insert(html_id.clone()) {
        records.push(object_record(
            doc,
            html_id.clone(),
            "publication_html",
    ...
    for asset in publication.assets {
        validate_key(&asset.path)?;
        let bytes = read_object(
            source_root,
            &format!(
                "publications/{}/objects/{}",
                doc.storage_id, asset.object.sha256
            ),
        )?;
        if bytes.len() > MAX_OBJECT_BYTES {
            return Err(Error::Invalid(format!(
                "publication asset {} exceeds object size limit",
                asset.path
            )));
        }
        if bytes.len() != asset.object.bytes || sha256(&bytes) != asset.object.sha256 {
            return Err(Error::Invalid(format!(
                "publication asset {} failed integrity",
                asset.path
            )));
        }
        let id = deterministic_id(
            "publication-asset",
            &format!("{}:{}", doc.storage_id, asset.object.sha256),
        );
        ...
        assets.push(json!({"path":asset.path,"sha256":asset.object.sha256,"bytes":asset.object.bytes,"mime":asset.object.mime,"object_id":id}));
    }
    let envelope = json!({"version":1,"publication_id":publication.publication_id,"bundle_sha256":publication.bundle_sha256,"source_sha256":publication.source_sha256,"render_config_sha256":publication.render_config_sha256,"published_at":publication.published_at,"publisher":publication.publisher,"previous_publication_id":publication.previous_publication_id,"html":{"sha256":publication.html.sha256,"bytes":publication.html.bytes,"mime":publication.html.mime,"object_id":html_id},"assets":assets});
    let body = json_text(&envelope, 262144, "publication manifest")?.into_bytes();
    let manifest_id = deterministic_id(
        "publication-manifest",
        &format!("{}:{}", doc.storage_id, publication.publication_id),
    );
    if record_ids.insert(manifest_id.clone()) {
        records.push(object_record(
            doc,
            manifest_id.clone(),
    ...
    let published_at = parse_time(&publication.published_at, "publication.published_at")?;
    let tx = target.transaction()?;
    for record in &records {
        insert_object(&tx, record, None)?;
    }
    tx.execute("UPDATE objects SET publication_root=1 WHERE document_id=?1 AND id IN (SELECT id FROM objects WHERE document_id=?1 AND kind IN ('publication_manifest','publication_html','publication_asset'))",[&doc.storage_id])?;
    tx.execute("UPDATE documents SET publication_id=?2,publication_object_id=?3,published_at=?4 WHERE id=?1",params![doc.storage_id,publication.publication_id,manifest_id,published_at])?;
    tx.commit()?;
    Ok(())
}
...
fn source_keyring_json(source: &Connection) -> Result<String> {
    ...
    let mut st = source
        .prepare("SELECT key_id,status,created_at,retired_at FROM link_keyring ORDER BY key_id")?;
    for row in st.query_map([], |r| {
        Ok((
            r.get::<_, String>(0)?,
            r.get::<_, String>(1)?,
...
fn recompute_counters(target: &mut Connection) -> Result<Counts> {
    ...
    tx.commit()?;
    Ok(Counts {
        accounts: target.query_row("SELECT COUNT(*) FROM accounts", [], |r| r.get::<_, i64>(0))?
            as u64,
        documents: values.2 as u64,
        grants: target.query_row("SELECT COUNT(*) FROM grants", [], |r| r.get::<_, i64>(0))? as u64,
        links: target.query_row("SELECT COUNT(*) FROM links", [], |r| r.get::<_, i64>(0))? as u64,
        annotations: target.query_row("SELECT COUNT(*) FROM annotations", [], |r| {
            r.get::<_, i64>(0)
        })? as u64,
        replies: target.query_row("SELECT COUNT(*) FROM replies", [], |r| r.get::<_, i64>(0))?
            as u64,
        checkpoints: target.query_row("SELECT COUNT(*) FROM checkpoints", [], |r| {
            r.get::<_, i64>(0)
        })? as u64,
        objects: target.query_row("SELECT COUNT(*) FROM objects", [], |r| r.get::<_, i64>(0))?
            as u64,
        checkpoint_objects: refs as u64,
        stored_bytes: values.0 as u64,
    })
}
...
fn verify_target(target_root: &Path, target: &Connection) -> Result<Verification> {
    ...
    for row in statement.query_map([], |r| {
        ...
        if state != "available" || length < 0 {
            return Err(Error::Invalid(format!(
                "target object {doc}/{id} is not settled"
            )));
        }
        let bytes = read_target_object(target_root, &key)?;
        if bytes.len() as i64 != length || sha256(&bytes) != digest {
            return Err(Error::Invalid(format!(
                "target object {key} failed final digest verification"
            )));
        }
    }
    let mut fk = target.prepare("PRAGMA foreign_key_check")?;
    if fk.query([])?.next()?.is_some() {
        return Err(Error::Invalid(
            "target foreign_key_check reported violations".into(),
        ));
    }
    let integrity: String = target.query_row("PRAGMA integrity_check", [], |r| r.get(0))?;
    if integrity != "ok" {
        return Err(Error::Invalid(format!(
            "target integrity_check failed: {integrity}"
        )));
    }
    ...
    for row in docs.query_map([], |r| {
        ...
        let actual_refs: i64 = target.query_row(
            "SELECT COUNT(*) FROM checkpoint_objects WHERE document_id=?1",
            [&id],
            |r| r.get(0),
        )?;
        if (stored, reserved, agent_bytes, agent_count) != (actual.0, actual.1, actual.2, actual.3)
            || refs != actual_refs
        {
            return Err(Error::Invalid(format!(
                "document {id} cached counters do not equal objects/checkpoint closure"
            )));
        }
        let journal: Option<String> = target
            .query_row(
            ...
            .optional()?
            .flatten();
        if let Some(object_id) = journal {
            let kind: String = target.query_row(
                "SELECT kind FROM objects WHERE document_id=?1 AND id=?2",
                params![id, object_id],
                |r| r.get(0),
            )?;
            if kind != "journal_base" {
                return Err(Error::Invalid(format!(
                    "document {id} journal base has wrong object kind"
                )));
            }
        }
    }
    // Account totals and document totals are independently defining rows.
    let mut accounts = target.prepare(
        "SELECT id,stored_bytes,reserved_bytes,document_count FROM accounts ORDER BY id",
    )?;
    for row in accounts.query_map([], |r| {
        Ok((
            r.get::<_, String>(0)?,
            r.get::<_, i64>(1)?,
            r.get::<_, i64>(2)?,
        ...
        let (id, stored, reserved, count) = row?;
        let actual:(i64,i64,i64)=target.query_row("SELECT COALESCE(SUM(stored_bytes),0),COALESCE(SUM(reserved_bytes),0),COUNT(*) FROM documents WHERE owner_id=?1",[&id],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?)))?;
        if (stored, reserved, count) != (actual.0, actual.1, actual.2) {
            return Err(Error::Invalid(format!(
                "account {id} cached counters do not equal documents"
            )));
        }
    }
    ...
    if counters != expected {
        return Err(Error::Invalid(
            "target cached server counters do not equal documents".into(),
        ));
    }
    let account_totals:(i64,i64,i64)=target.query_row("SELECT COALESCE(SUM(stored_bytes),0),COALESCE(SUM(reserved_bytes),0),COALESCE(SUM(document_count),0) FROM accounts",[],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?)))?;
    if account_totals != (expected.0, expected.1, expected.2) {
        return Err(Error::Invalid(
            "account totals do not equal document totals".into(),
        ));
    }
    ...
    let mut checkpoints = target
        .prepare("SELECT document_id,id,tree_object_id FROM checkpoints ORDER BY document_id,id")?;
    for row in checkpoints.query_map([], |r| {
        Ok((
            r.get::<_, String>(0)?,
            r.get::<_, String>(1)?,
            r.get::<_, String>(2)?,
        ...
        let (doc, id, tree) = row?;
        let tree_count:i64=target.query_row("SELECT COUNT(*) FROM checkpoint_objects WHERE document_id=?1 AND checkpoint_id=?2 AND object_id=?3",params![doc,id,tree],|r|r.get(0))?;
        if tree_count != 1 {
            return Err(Error::Invalid(format!(
                "checkpoint {doc}/{id} is missing its tree object closure"
            )));
        }
        let refs:i64=target.query_row("SELECT COUNT(*) FROM checkpoint_objects co JOIN objects o ON o.document_id=co.document_id AND o.id=co.object_id WHERE co.document_id=?1 AND co.checkpoint_id=?2 AND o.state='available'",params![doc,id],|r|r.get(0))?;
        let declared: i64 = target.query_row(
            "SELECT COUNT(*) FROM checkpoint_objects WHERE document_id=?1 AND checkpoint_id=?2",
            params![doc, id],
            |r| r.get(0),
        )?;
        if refs != declared {
            return Err(Error::Invalid(format!(
                "checkpoint {doc}/{id} references an unavailable object"
            )));
        }
    }
    Ok(Verification {
        source_integrity: "ok".into(),
        target_integrity: "ok".into(),
        physical_digests: true,
        foreign_keys: true,
        counters: true,
...
fn read_target_object(root: &Path, key: &str) -> Result<Vec<u8>> {
    validate_key(key)?;
    let path = objects_path(root).join(key);
    reject_path_symlinks(&objects_path(root), &path)?;
    Ok(fs::read(path)?)
...
fn print_plan(source_id: &str, schema: &str, plan: &Plan, manifest: Option<&Manifest>) {
    let report = json!({"source_identity":source_id,"source_schema_fingerprint":schema,"documents":plan.documents.iter().map(|d|json!({"storage_id":d.storage_id,"slug":d.slug,"title":d.title,"source_format":d.source_format})).collect::<Vec<_>>(),"excluded":plan.exclusions,"errors":plan.errors,"active_link_key_id":plan.active_key,"would_write":!plan.documents.is_empty()});
    println!(
        "{}",
...
fn run(args: Args) -> Result<()> {
    let source = canonical_root(&args.source_data)?;
    let target = canonical_root(&args.target_data)?;
    if overlaps(&source, &target) {
        return Err(Error::Invalid("source and target paths overlap".into()));
    }
    reject_symlinks(&source)?;
    reject_symlinks(&target)?;
    let source_catalog = catalog_path(&source);
    ...
    target_empty_or_resume(&target, args.resume)?;
    let _source_lock = Lock::source(&read_lock_path(&source))?;
    let source_db = open_source(&source_catalog)?;
    let (schema, _integrity) = validate_source(&source_db)?;
    let source_digest = catalog_digest(&source_catalog)?;
    ...
    if args.dry_run {
        ...
        if !plan.errors.is_empty() {
            return Err(Error::Invalid(plan.errors.join("; ")));
        }
        return Ok(());
    }
    if !plan.errors.is_empty() {
    ...
    fs::create_dir_all(&target)?;
    let _target_lock = Lock::target(&read_lock_path(&target))?;
    let manifest_path = target.join(MANIFEST_FILE);
    let target_id;
    let mut manifest;
    let mut target_db = if args.resume {
        let bytes = fs::read(&manifest_path)?;
        manifest = serde_json::from_slice::<Manifest>(&bytes)?;
        if manifest.source_identity != source_id
            || manifest.source_schema_fingerprint != schema
            || manifest.source_catalog_digest != source_digest
            || manifest.source_physical_digest != physical_digest
        {
            return Err(Error::Invalid(
                "resume manifest does not match source identity/schema/catalog/physical snapshot"
                    .into(),
            ));
        }
        target_id = manifest.target_identity.clone();
        open_target_existing(&target, &manifest)?
    } else {
        target_id = target_identity(&target, &source_id)?;
        manifest = new_manifest(
            &source,
            &target,
            source_id.clone(),
            source_digest.clone(),
            physical_digest.clone(),
            schema.clone(),
            target_id.clone(),
            &plan,
            &args.documents,
    ...
    if target_db.query_row::<i64, _, _>(
        "SELECT COUNT(*) FROM server_state WHERE id=1",
        [],
        |r| r.get(0),
    )? != 1
    {
        return Err(Error::Invalid(
            "target server_state singleton is missing".into(),
        ));
    }
    let _secret_count = copy_secrets(&source, &target)?;
    import_keyring(&source_db, &mut target_db, &plan.active_key)?;
    insert_accounts(&source_db, &mut target_db, &plan.documents, &target_id)?;
    insert_documents(&source_db, &mut target_db, &plan.documents)?;
    import_sharing(&source_db, &mut target_db, &plan.documents)?;
    manifest.phase = "static".into();
    ...
    for doc in &plan.documents {
        let p = manifest
            .documents
            .entry(doc.storage_id.clone())
            .or_default();
        if p.status == "complete" {
            continue;
        }
        if let Err(e) = convert_document(&source, &target, &source_db, &mut target_db, doc, p) {
            p.status = "error".into();
            p.error = Some(e.to_string());
            manifest.errors.push(format!("{}: {e}", doc.storage_id));
            sync_json(&manifest_path, &manifest)?;
            return Err(e);
        }
        p.status = "complete".into();
        p.cursor = "document-complete".into();
        manifest.updated_at = now_ms();
        sync_json(&manifest_path, &manifest)?;
    }
    for doc in &plan.documents {
        convert_publication(&source, &source_db, &target, &mut target_db, doc)?;
    }
    import_annotations(&source_db, &mut target_db, &plan.documents, &mut progress)?;
    for (doc_id, annotation_progress) in progress {
        if let Some(saved) = manifest.documents.get_mut(&doc_id) {
            saved.annotations = annotation_progress.annotations;
        }
    }
    manifest.phase = "reconciled".into();
    manifest.counts = recompute_counters(&mut target_db)?;
    manifest.verification = verify_target(&target, &target_db)?;
    manifest.verification.complete_manifest = true;
    manifest.complete = true;
    manifest.phase = "complete".into();
    manifest.updated_at = now_ms();
    sync_json(&manifest_path, &manifest)?;
    println!("conversion complete: {}", manifest_path.display());
    Ok(())
}
...
mod tests {
    ...
    fn object_write_is_idempotent_and_never_overwrites_different_bytes() {
        ...
        let interrupted = root
            .path()
            .join("objects/v2/documents/doc/objects/.next.tmp");
        std::fs::write(&interrupted, b"partial").expect("interrupted staging inode");
        write_object(root.path(), "v2/documents/doc/objects/next", b"complete")
            .expect("resume staging");
        assert_eq!(
            std::fs::read(root.path().join("objects/v2/documents/doc/objects/next"))
                .expect("resumed object"),
            b"complete"
        );
    }
    ...
    fn converts_fixture_document_and_resume_is_idempotent() {
        ...
        std::fs::create_dir_all(source.path().join("state")).expect("state");
        std::fs::create_dir_all(source.path().join("objects")).expect("objects");
        std::fs::write(source.path().join("state/writer.lock"), b"").expect("writer lock");
        let source_db = Connection::open(source.path().join("catalog.db")).expect("catalog");
        source_db.execute_batch(V1_DDL).expect("v1 fixture");
        source_db
        ...
        let base_payload = ydoc
            .transact()
            .encode_state_as_update_v1(&yrs::StateVector::default());
        let base_vector = ydoc.transact().state_vector();
        {
            let mut txn = ydoc.transact_mut();
            text.insert(&mut txn, 5, " world");
        }
        let update_payload = ydoc.transact().encode_state_as_update_v1(&base_vector);
        let base_bytes = encode_recovery_base("doc-1", 0, 1, &base_payload).expect("base envelope");
        let base_key = "journal/base";
        std::fs::create_dir_all(source.path().join("objects/journal")).expect("journal dir");
        std::fs::write(source.path().join("objects").join(base_key), &base_bytes).expect("base");
        let mut segment = Vec::new();
        segment.extend_from_slice(b"KJNL");
        segment.extend_from_slice(&2u16.to_le_bytes());
        segment.extend_from_slice(&1u32.to_le_bytes());
        segment.extend_from_slice(&2u16.to_le_bytes());
        segment.extend_from_slice(&("doc-1".len() as u16).to_le_bytes());
        segment.extend_from_slice(b"doc-1");
        segment.extend_from_slice(&2u64.to_le_bytes());
        segment.extend_from_slice(&0u64.to_le_bytes());
        segment.extend_from_slice(&0u32.to_le_bytes());
        segment.extend_from_slice(&1u32.to_le_bytes());
        let retry = "fixture-retry";
        segment.extend_from_slice(&(retry.len() as u16).to_le_bytes());
        segment.extend_from_slice(retry.as_bytes());
        let update_digest = sha256(&update_payload);
        segment.extend_from_slice(&(update_digest.len() as u16).to_le_bytes());
        segment.extend_from_slice(update_digest.as_bytes());
        segment.extend_from_slice(&(update_digest.len() as u16).to_le_bytes());
        segment.extend_from_slice(update_digest.as_bytes());
        segment.extend_from_slice(&(update_payload.len() as u32).to_le_bytes());
        segment.extend_from_slice(&update_payload);
        let segment_key = "journal/shared";
        std::fs::write(source.path().join("objects").join(segment_key), &segment).expect("segment");
        source_db.execute("INSERT INTO journal_bases(base_id,storage_id,epoch,sequence,object_key,digest,encoded_bytes,committed_at) VALUES('base-1','doc-1',0,1,?1,?2,?3,0)",params![base_key,sha256(&base_bytes),base_bytes.len() as i64]).expect("journal base");
        source_db.execute("INSERT INTO journal_segments(segment_id,segment_seq,operation_id,object_key,digest,encoded_bytes,committed_at) VALUES('segment-1',1,'op-1',?1,?2,?3,0)",params![segment_key,sha256(&segment),segment.len() as i64]).expect("journal segment");
        let source_bytes = b"# hello\n";
        let source_sha = sha256(source_bytes);
        std::fs::create_dir_all(source.path().join("objects/content/doc-1/blobs"))
            .expect("source blob dir");
        std::fs::write(
            source
                .path()
                .join(format!("objects/content/doc-1/blobs/{source_sha}")),
            source_bytes,
        )
        .expect("source blob");
        let tree = json!({"main":"paper.md","files":{"paper.md":{"kind":"text","id":"file-1","sha":source_sha,"size":source_bytes.len()}},"settings":{"engine":"markdown"}});
        let tree_bytes = serde_json::to_vec(&tree).expect("tree");
        let tree_sha = sha256(&tree_bytes);
        std::fs::create_dir_all(source.path().join("objects/content/doc-1/trees"))
            .expect("source tree dir");
        std::fs::write(
            source
                .path()
                .join(format!("objects/content/doc-1/trees/{tree_sha}")),
            tree_bytes,
        )
        .expect("source tree");
        source_db.execute("INSERT INTO checkpoints(slug,sha,seq,durable_seq,tree_sha,parent,at,by,why,source_format,size,label,git_commit,dirty,changed,by_account) VALUES('paper','cp-1',1,1,?1,'','2026-01-01T00:00:00Z','Vincent','initial','markdown',?2,'','',0,'[]','acct')",params![tree_sha,source_bytes.len() as i64]).expect("checkpoint");
        let args = Args {
        ...
        run(args).expect("conversion");
        let target_db =
            Connection::open(target.path().join("v2/catalog.db")).expect("target catalog");
        assert_eq!(
            target_db
        ...
        assert_eq!(
            target_db
                .query_row::<i64, _, _>("SELECT COUNT(*) FROM documents", [], |r| r.get(0))
                .expect("document"),
            1
        );
        assert!(target.path().join("v2/conversion-manifest.json").is_file());
        let (tree_id, tree_digest): (String, String) = target_db
            .query_row(
                "SELECT tree_object_id,tree_digest FROM checkpoints",
                [],
                |r| Ok((r.get(0)?, r.get(1)?)),
            )
            .expect("checkpoint");
        let tree_key: String = target_db
            .query_row(
                "SELECT storage_key FROM objects WHERE id=?1",
                [&tree_id],
                |r| r.get(0),
            )
            .expect("tree object");
        let tree_wire: V2TreeEnvelope = serde_json::from_slice(
            &read_target_object(&target.path().join("v2"), &tree_key).expect("tree bytes"),
        )
        .expect("v2 tree envelope");
        assert_eq!(tree_wire.version, 1);
        assert_eq!(tree_digest, hex::encode(tree_wire.logical_digest));
        let source_file = tree_wire.files.get("paper.md").expect("main file");
        let recipe = source_file.recipe.as_ref().expect("recipe locator");
        let recipe_bytes = read_target_object(
            &target.path().join("v2"),
            &format!("v2/documents/doc-1/objects/{}", recipe.object_id),
        )
        .expect("recipe bytes");
        let recipe_wire: V2RecipeEnvelope =
            serde_json::from_slice(&recipe_bytes).expect("v2 recipe envelope");
        assert_eq!(recipe_wire.recipe.file_digest, source_file.logical_digest);
        let journal_key: String = target_db
            .query_row(
                "SELECT storage_key FROM objects WHERE kind='journal_base'",
                [],
                |r| r.get(0),
            )
            .expect("journal base");
        let journal =
            read_target_object(&target.path().join("v2"), &journal_key).expect("journal bytes");
        let (_, epoch, sequence, payload) = decode_base(&journal).expect("recovery base envelope");
        assert_eq!((epoch, sequence), (0, 2));
        let replay_doc = Doc::new();
        replay_doc
            .transact_mut()
            .apply_update(Update::decode_v1(&payload).expect("decoded replay"))
            .expect("applied replay");
        let replay_text = replay_doc.get_or_insert_text("body");
        let replay_txn = replay_doc.transact();
        assert_eq!(replay_text.get_string(&replay_txn), "hello world");
        let args = Args {
            source_data: source.path().to_path_buf(),
            target_data: target.path().join("v2"),
            dry_run: false,
        ...
        source_db
            .execute(
                "UPDATE documents SET title='Changed' WHERE slug='paper'",
                [],
            )
    ...
    fn dry_run_does_not_create_target_root() {
        ...
        let args = Args {
            source_data: source.path().to_path_buf(),
            target_data: target.path().join("new"),
            dry_run: true,
            resume: false,
            documents: Vec::new(),
            active_link_key: None,
    ...
    fn source_history_reconstructs_actual_v1_multi_chunk_recipe() {
        ...
        std::fs::create_dir_all(root.path().join("objects/content/doc-1/recipes"))
            .expect("recipe dir");
        std::fs::create_dir_all(root.path().join("objects/content/doc-1/chunks"))
            .expect("chunk dir");
        ...
        source_bytes.extend(std::iter::repeat(b'a').take(300_000));
        source_bytes.extend(std::iter::repeat(b'b').take(300_000));
        let split = 300_000;
        let first = &source_bytes[..split];
        let second = &source_bytes[split..];
        let file_digest = sha256(&source_bytes);
        let first_digest = sha256(first);
        let second_digest = sha256(second);
        let mut recipe = Vec::new();
        recipe.extend_from_slice(b"LPREC001");
        recipe.extend_from_slice(&1u16.to_le_bytes());
        recipe.extend_from_slice(&1u16.to_le_bytes());
        recipe.push(2);
        recipe.push(0);
        recipe.extend_from_slice(&(source_bytes.len() as u64).to_le_bytes());
        recipe.extend_from_slice(&hex::decode(&file_digest).expect("file digest"));
        recipe.extend_from_slice(&2u32.to_le_bytes());
        recipe.extend_from_slice(&hex::decode(&first_digest).expect("first digest"));
        recipe.extend_from_slice(&(first.len() as u32).to_le_bytes());
        recipe.extend_from_slice(&hex::decode(&second_digest).expect("second digest"));
        recipe.extend_from_slice(&(second.len() as u32).to_le_bytes());
        let recipe_key = format!("content/doc-1/recipes/{file_digest}");
        std::fs::write(root.path().join("objects").join(&recipe_key), &recipe).expect("recipe");
        let first_encoded = zstd::stream::encode_all(first, 3).expect("first chunk");
        let second_encoded = zstd::stream::encode_all(second, 3).expect("second chunk");
        let first_key = format!("content/doc-1/chunks/{first_digest}");
        let second_key = format!("content/doc-1/chunks/{second_digest}");
        std::fs::write(root.path().join("objects").join(&first_key), &first_encoded)
            .expect("first chunk");
        std::fs::write(
            root.path().join("objects").join(&second_key),
            &second_encoded,
        )
        .expect("second chunk");
        db.execute("INSERT INTO source_history_encodings(storage_id,file_digest,recipe_key,recipe_digest,codec,uncompressed_bytes,recipe_bytes,created_at) VALUES('doc-1',?1,?2,?3,2,?4,?5,0)",params![file_digest,recipe_key,sha256(&recipe),source_bytes.len() as i64,recipe.len() as i64]).expect("encoding row");
        db.execute("INSERT INTO source_history_objects(storage_id,file_digest,object_key,kind,bytes) VALUES('doc-1',?1,?2,'source_chunk',?3),('doc-1',?1,?4,'source_chunk',?5)",params![file_digest,first_key,first_encoded.len() as i64,second_key,second_encoded.len() as i64]).expect("object rows");
        ...
        let restored = source_history_file(root.path(), &db, &doc, &file_digest)
    ...
    fn converts_publication_annotations_replies_and_secret_files() {
        ...
        let target = tempfile::tempdir().expect("target");
        std::fs::create_dir_all(source.path().join("state")).expect("state");
        std::fs::create_dir_all(source.path().join("objects/publications/doc-1/objects"))
        ...
        let html = b"<html>published</html>";
        let html_sha = sha256(html);
        let asset = b"shared asset";
        let asset_sha = sha256(asset);
        std::fs::write(
            source
                .path()
                .join(format!("objects/publications/doc-1/objects/{html_sha}")),
            html,
        )
        .expect("html");
        std::fs::write(
            source
                .path()
                .join(format!("objects/publications/doc-1/objects/{asset_sha}")),
            asset,
        )
        .expect("asset");
        let publication = json!({"publication_id":"pub-1","bundle_sha256":sha256(b"bundle"),"source_sha256":sha256(b"source"),"render_config_sha256":sha256(b"config"),"published_at":"2","publisher":"acct","previous_publication_id":"","html":{"sha256":html_sha,"bytes":html.len(),"mime":"text/html"},"assets":[{"path":"img/a.png","sha256":asset_sha,"bytes":asset.len(),"mime":"image/png"},{"path":"img/b.png","sha256":asset_sha,"bytes":asset.len(),"mime":"image/png"}]});
        std::fs::write(
            source
                .path()
                .join("objects/publications/doc-1/current.json"),
        ...
        let resolved_provenance: (Option<String>, Option<String>, Option<String>) = target_db
            .query_row(
                "SELECT source_revision,resolution_revision,protected_checkpoint_id
                 FROM annotations WHERE id='comment-2'",
                [],
                |r| Ok((r.get(0)?, r.get(1)?, r.get(2)?)),
            )
            .expect("resolved provenance");
        assert_eq!(
            resolved_provenance,
            (
                Some("original-source".into()),
                Some("resolution-checkpoint".into()),
                None
            )
        );
        ...
        assert_eq!(
            target_db
                .query_row::<i64, _, _>(
                    "SELECT COUNT(*) FROM objects WHERE publication_root=1",
                    [],
                    |r| r.get(0)
                )
