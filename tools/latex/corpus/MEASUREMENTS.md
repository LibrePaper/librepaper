...
`tools/latex/corpus/pages.json`, written by `node tools/latex/tools/texlive.mjs`.
...
`tools/latex/corpus/packages/` asks for, in one document, in two pages, matching a
...
`tools/latex/corpus/packages/` asks for -- compiles to its two pages through
