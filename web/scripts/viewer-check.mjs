...
//
// `05-SPEC-latex.md`'s bet, in one sentence: "a LaTeX document is a hard case
...
// `node latex/texlive.mjs --pdf` is what makes it run.
//
...
const SHELL = join(REPO, "src", "shell");
...
if (!existsSync(join(SHELL, "viewer.html"))) {
  console.log("viewer: no built shell at src/shell; skipping (run `make web`)");
  process.exit(0);
}
if (!existsSync(ARTICLE) || !existsSync(PAPER)) {
  console.log("viewer: no corpus PDFs; skipping (run `node latex/texlive.mjs --pdf`)");
  process.exit(0);
}
