...
// against `latex/serve.mjs` on a temporary port.
//
...
// do without `node_modules`: `node latex/mirror.mjs` is what makes it run.
//
// Usage: latex-check.mjs [--only <distribution>] [--measure]
//   --measure writes examples/latex/MEASUREMENTS.md from what it saw.
...
const CORPUS = join(REPO, "examples", "latex");
...
if (!existsSync(join(MIRROR, "manifest.json"))) {
  console.log("latex: no mirror at latex/mirror; skipping (run `node latex/mirror.mjs`)");
  process.exit(0);
}
...
if (MEASURE) {
  ...
  console.log(`latex: measurements written to examples/latex/MEASUREMENTS.md`);
}
