...
try {
  ...
  const mode = async value => tab.evaluate(`(() => {
    const select = document.querySelector('[aria-label="Compare selected checkpoint"]');
    select.value = ${JSON.stringify(value)}; select.dispatchEvent(new Event('change', {bubbles:true}));
  })()`);
  await select(1);
  await mode('previous'); await expectDiff(sources[0], sources[1]);
  await select(2); await expectDiff(sources[1], sources[2]);
  await mode('checkpoint');
  await until('checkpoint source alone', () => tab.evaluate(`document.querySelectorAll('.cm-content').length === 1 && document.querySelector('.cm-content').textContent === ${JSON.stringify(sources[2])}`), 4000);
  await select(0);
  await until('another checkpoint source', () => tab.evaluate(`document.querySelectorAll('.cm-content').length === 1 && document.querySelector('.cm-content').textContent === ${JSON.stringify(sources[0])}`), 4000);
  await tab.evaluate(`document.querySelector('.timeline-now .timeline-point').click()`);
  await until('current source after historical navigation', () => tab.evaluate(`document.querySelectorAll('.cm-content').length === 1 && document.querySelector('.cm-content').textContent === ${JSON.stringify(current)}`), 4000);
  await select(1);
  await until('mode survives current row', () => tab.evaluate(`document.querySelectorAll('.cm-content').length === 1 && document.querySelector('.cm-content').textContent === ${JSON.stringify(sources[1])}`), 4000);
  console.log('history reader: all comparison modes, current row, and repeated navigation');
  await tab.evaluate(`document.querySelector('.sidebar-activity [aria-label="Files"]').click()`);
  await tab.evaluate(`document.querySelector('.sidebar-activity [aria-label="History"]').click()`);
  await select(1);
  await until('reopened history matches its checkpoint mode', () => tab.evaluate(`document.querySelector('[aria-label="Compare selected checkpoint"]').value === 'checkpoint' && document.querySelectorAll('.cm-content').length === 1 && document.querySelector('.cm-content').textContent === ${JSON.stringify(sources[1])}`), 4000);
  await tab.evaluate(`document.querySelector('.timeline-folded').click()`);
  await until('grouped row uses checkpoint mode', () => tab.evaluate(`document.querySelectorAll('.cm-content').length === 1 && document.querySelector('.cm-content').textContent === ${JSON.stringify(sources[5])}`), 4000);
  console.log('history reader: grouped and individual rows use the same mode');
  ...
  await mode('previous'); await expectDiff(sources[0], sources[1]);
  assert.equal(await tab.evaluate(`document.querySelector('.reader')?.classList.contains('no-preview')`), true, 'all history modes hide the preview');
  assert.equal(await tab.evaluate(`window.historyLive.source()`), current, 'history navigation never modifies the live source');
  await mode('current'); await expectDiff(sources[1], current);
  const addition = '<p>live update</p>';
  await tab.evaluate(`window.historyLive.append(${JSON.stringify(addition)})`);
  assert.equal(await tab.evaluate(`document.querySelector('.cm-merge-b .cm-content')?.textContent`), current, 'the current comparison is a stable snapshot');
  await tab.evaluate(`([...document.querySelectorAll('.history-workspace button')].find(button=>button.textContent==='Refresh current')).click()`);
