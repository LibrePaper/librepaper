...
const shell = `<!doctype html><script type="module">
  import { buildDisplayBundle } from "/src/lib/bundle-builder.js";
...
let server, page;
const temporary = mkdtempSync(join(tmpdir(), "librepaper-bundle-builder-"));
...
try {
  ...
  await until("bundle builder loaded", () => page.evaluate("typeof window.bundle === 'function'"), 5000);
  ...
  console.log("bundle-builder-browser: public dependency closure, binary deduplication and actionable rejections passed");
} finally {
  await page?.close?.();
  server?.close();
