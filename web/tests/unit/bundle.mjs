...
import { createBundlePublisher, createBundleReader, digestBundle, sha256 } from "../../src/lib/bundle.js";

const bytes = new TextEncoder().encode("hello");
const bundle = await digestBundle({ html: "<p>hello</p>", assets: [{ path: "public/a.txt", bytes, mime: "text/plain" }] });
...
const fetcher = async (path, options = {}) => {
  ...
  if (path.endsWith("/activate")) return new Response(JSON.stringify({ bundle: { id: "pub-1" } }), { status: 200 });
  return new Response(JSON.stringify({ bundle: null }), { status: 200 });
};
const published = await createBundlePublisher({ slug: "paper", fetcher }).publish({ html: "<p>hello</p>", assets: [{ path: "public/a.txt", bytes }], sourceRevision: "a".repeat(64) });
...
assert.equal(prepareBody.expected_bundle_id, null);
...
assert.deepEqual(Object.keys(JSON.parse(activate.options.body)).sort(), ["expected_bundle_id", "manifest"]);

// A warm bundle reuses every object the server says it already owns.
const warmCalls = [];
const warmFetcher = async (path, options = {}) => {
  ...
  if (path.endsWith("/activate")) return new Response(JSON.stringify({ bundle: { id: "pub-2" } }), { status: 200 });
  return new Response("{}", { status: 200 });
};
await createBundlePublisher({ slug: "paper", fetcher: warmFetcher }).publish({ html: "<p>hello</p>", assets: [{ path: "public/a.txt", bytes }], sourceRevision: "b".repeat(64) });
...
if (typeof CompressionStream === "function" && typeof DecompressionStream === "function") {
  const gzipCalls = [];
  const longHtml = `<p>${"bundle text ".repeat(800)}</p>`;
  const gzipFetcher = async (path, options = {}) => {
    ...
    if (path.endsWith("/activate")) return new Response(JSON.stringify({ bundle: { id: "pub-gzip" } }), { status: 200 });
    return new Response("{}", { status: 200 });
  };
  await createBundlePublisher({ slug: "paper", fetcher: gzipFetcher }).publish({ html: longHtml });
...
let shown;
const reader = createBundleReader({ slug: "paper", fetcher, onBundle: (value) => { shown = value; } });
await reader.refresh();
assert.equal(shown, null);
assert.ok(calls.every(({ path }) => !/\/source$|\/snapshot$|\/state$|\/history/.test(path)), "reader bundle refresh never requests source state");
assert.equal(reader.announce({ id: "pub-2" }), true);
assert.equal(shown.pending_id, "pub-2");
assert.equal(shown.id, undefined, "announcement preserves the draft without replacing current bundle metadata");
reader.dispose();
console.log("bundle: digest, allowlist, missing-object upload and refresh behavior passed");
...
const costFetcher = async (path, options = {}) => {
  if (path.endsWith("/prepare")) {
    ...
    const { manifest, expected_bundle_id } = JSON.parse(options.body);
    assert.equal(expected_bundle_id, active?.id ?? null);
  ...
  return Response.json({ bundle: active });
};
function encoderBytes(value) { return new TextEncoder().encode(value).byteLength; }
const costPublisher = createBundlePublisher({ slug: "cost", fetcher: costFetcher });
...
for (const [index, word] of ["original", "changed", "changed"].entries()) {
  ...
  await costPublisher.publish({ html: `<p>${word}</p><img loading="lazy" src="assets/${imageHash}.png">`, assets: [{ path: `assets/${imageHash}.png`, mime: "image/png", bytes: imageBytes }], sourceRevision: String(index + 1).repeat(64), expectedBundleId: active?.id ?? null });
...
console.log("bundle cost fixture (object bytes; HTTP overhead excluded):", JSON.stringify(measurements));
