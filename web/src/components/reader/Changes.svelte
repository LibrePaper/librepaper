...
<style>
  ...
  .changes-title-row, .filters, .row-head { display: flex; align-items: center; gap: var(--spacing); } .changes-title-row { justify-content: space-between; } h2 { margin: 0; font-size: 1rem; }
  ...
  .filter-bar { display: flex; gap: .4rem; margin-top: .65rem; } .filter-bar select { min-width: 7rem; } .filter-bar .changes-menu { margin-left: auto; position: relative; } .changes-menu summary { list-style: none; cursor: pointer; padding: .2rem .45rem; border-radius: .25rem; } .changes-menu summary::-webkit-details-marker { display: none; } .menu-popover { position: absolute; right: 0; top: calc(100% + .25rem); width: 13rem; padding: .7rem; display: grid; gap: .45rem; background: var(--color-surface-50-950); border: 1px solid var(--color-surface-200-800); border-radius: .4rem; box-shadow: var(--shadow-xl); } .menu-popover label { display: flex; gap: .4rem; align-items: center; font-size: .8rem; } .menu-popover hr { width: 100%; border: 0; border-top: 1px solid var(--color-surface-200-800); }
