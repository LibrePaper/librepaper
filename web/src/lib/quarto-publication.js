...
export async function publishQuartoBundle(api, payload, knownManifest = null) {
  ...
  let response = await api.quartoPublish(reduced);
  ...
  if (response.status === 400 && reduced.blobs.length !== payload.blobs.length) response = await api.quartoPublish(payload);
  if (response.ok) return response.json();
  if (response.status === 409) {
    const saved = await api.quartoBundle(payload.manifest.render_id);
    if (saved.ok) {
