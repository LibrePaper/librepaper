// Turn a complete, authenticated bundle into browser-local display resources.
...
const ROOT = "https://quarto-artifact.invalid/";
...
export function artifactReference(value, containingPath) {
...
async function verified(bytes, descriptor) {
  ...
  if (value.byteLength !== Number(descriptor.size)) throw new Error(`Saved Quarto resource has the wrong size: ${descriptor.path || descriptor.entrypoint}`);
  const digest = [...new Uint8Array(await crypto.subtle.digest("SHA-256", value))].map((part) => part.toString(16).padStart(2, "0")).join("");
  if (digest !== descriptor.sha256) throw new Error(`Saved Quarto resource has the wrong digest: ${descriptor.path || descriptor.entrypoint}`);
  return value;
}

export async function prepareQuartoArtifact(manifest, artifactBytes, readAsset) {
  if (!manifest) throw new Error("The saved Quarto bundle is missing.");
  const urls = [];
  ...
  try {
    ...
    await Promise.all(Array.from({ length:Math.min(6, inventory.length) }, async () => {
      while (next < inventory.length) {
        ...
        if (downloaded == null) throw new Error(`Saved Quarto resource is missing: ${asset.path}`);
    ...
    function reference(value, containingPath, required = true) {
      const resolved = artifactReference(value, containingPath);
      if (!resolved) return value;
      if (!descriptors.has(resolved.path)) {
        if (required) throw new Error(`Saved Quarto resource is missing: ${resolved.path}`);
        return value;
      }
