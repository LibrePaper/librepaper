import { sha256 } from "./quarto.js";
import { prepareQuartoArtifact } from "./quarto-artifact.js";

export function resultItems(manifest) {
...
export function resultAnchor(manifest, item, dimensions = {}) {
...
export async function inspectResult(api, anchor) {
  const response = await api.quartoBundle(anchor.render_id);
  ...
  const item = resultItems(manifest).find((item) => item.cell.id === anchor.cell_id && item.output.ordinal === anchor.output_ordinal);
  ...
  const prepared = await prepareQuartoArtifact({ ...manifest, artifact:null, assets }, null, async (asset) => {
    const resource = await api.quartoAsset(anchor.render_id, asset.path);
