...
// broad regex so the bundle closure follows only CSS tokens that load a
// resource.
...
export async function buildDisplayBundle(html, tree, { fetcher = globalThis.fetch, parse = (value) => new DOMParser().parseFromString(value, "text/html") } = {}) {
  ...
  // bundle. Its effects are resolved against the captured main instead.
  ...
  function resolvePath(reference, from) {
    ...
    const root = new URL(from, "https://bundle.invalid/");
