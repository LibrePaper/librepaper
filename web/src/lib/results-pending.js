...
function valid(slug, payload) {
  try {
    return typeof slug === "string" && slug.length > 0 &&
      ...
      Array.isArray(payload.blobs) && validateResultsManifest(payload.manifest) === "quarto";
  } catch {
    return false;
  }
}
...
function open() {
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => reject(request.error || new Error("Could not open the results outbox."));
    request.onblocked = () => reject(new Error("Another tab is blocking the results outbox."));
  });
}
...
export async function savePendingResults(slug, payload) {
  ...
  if (JSON.stringify(payload).length > MAX_CHARACTERS) throw new Error("Completed result is too large for the browser outbox; keep this tab open to retry sharing.");
  const now = Date.now();
  return transaction("readwrite", (store, done, fail) => {
    ...
    request.onsuccess = () => {
      ...
      if (existing && existing.payload.manifest.render_id !== payload.manifest.render_id) {
        fail(new Error("Another completed result for this document is waiting to share; keep this tab open to retain this result."));
        return;
      }
      if (!retained.some((record) => record.slug === slug) && retained.length >= MAX_DOCUMENTS) {
        fail(new Error("The results outbox has eight documents waiting to share; keep this tab open and share those results first."));
        return;
      }
...
export async function loadPendingResults(slug) {
...
export async function clearPendingResults(slug, renderId) {
  return transaction("readwrite", (store, done) => {
    const request = store.get(slug);
    request.onsuccess = () => {
