import { prepareQuartoArtifact } from "./quarto-artifact.js";
...
export function createQuartoLoader({ api, apply, prepare = prepareQuartoArtifact }) {
  let epoch = 0;
  let flight = null;
  let loadedContext = "";
  let value = null;
  return {
    ...
    load(context, { force = false } = {}) {
      ...
      const serial = ++epoch;
      const promise = (async () => {
        const response = await api.quartoSelected(context);
        const descriptor = await response.json().catch(() => null);
        if (serial !== epoch) return null;
        if (!response.ok && response.status !== 404) throw new Error(descriptor?.error || "Saved Quarto output could not be loaded.");
        ...
        if (manifest.artifact) {
          const artifact = await api.quartoArtifact(manifest.render_id);
          if (!artifact.ok) throw new Error("The saved Quarto artifact could not be loaded.");
          artifactBytes = new Uint8Array(await artifact.arrayBuffer());
        }
        const prepared = await prepare(manifest, artifactBytes, async (asset) => {
          const response = await api.quartoAsset(manifest.render_id, asset.path);
          if (!response.ok) throw new Error(`Saved Quarto resource is missing: ${asset.path}`);
