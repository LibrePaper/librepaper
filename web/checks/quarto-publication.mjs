...
import { publishResultsBundle } from "../src/lib/results-publication.js";
...
const calls = [];
const receipt = await publishResultsBundle({
  publishResults: async (body) => { calls.push(body); return { ok:false, status:409 }; },
  resultBundle: async () => ({ ok:true, json:async () => structuredClone(payload.manifest) }),
...
await assert.rejects(publishResultsBundle({
  publishResults: async () => ({ ok:false, status:409, json:async () => ({ error:"render ID conflict" }) }),
  resultBundle: async () => ({ ok:true, json:async () => ({ ...payload.manifest, context:{ id:"pdf" } }) }),
}, payload), /render ID conflict/);
for (const status of [400, 404]) {
const fallback = [];
await publishResultsBundle({ publishResults: async (body) => {
  fallback.push(body);
