...
ctx.openFile = "b";
const asking = vm.runInContext("askAssistant()", ctx);
completeDigest("revision-a");
await asking;
assert.equal(capturedTrees[0].main, "a.md");
assert.equal(ctx.assistantRequest.selection.source.path, "a.md");
assert.equal(ctx.assistantRequest.revision, "revision-a");
