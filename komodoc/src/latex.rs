...
//! `latex/mirror.mjs` -- so it is immutable for a year and a new release is a
...
#[derive(Debug)]
pub enum Mirror {
    /// A directory on this machine, the shape `latex/mirror.mjs` writes.
    Directory(PathBuf),
        ...
        if !path.is_dir() {
            return Err(format!(
                "--latex {flag} is neither an https URL nor a directory on this machine.\n\n  \
                 Build one with `node latex/mirror.mjs`, or pass the bucket that serves it."
            ));
        }
        ...
        match self {
            Mirror::Directory(root) => {
                if root.join(MANIFEST).is_file() {
                    ...
                    Some(format!(
                        "warning: {} has no manifest.json; run `node latex/mirror.mjs` to build one",
                        root.display()
                    ))
                }
            }
