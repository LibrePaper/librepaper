...
const pages = await (await fetch('http://127.0.0.1:9704/json/list')).json();
const page = pages.find(p => p.url.startsWith('http://127.0.0.1:8704/'));
...
try {
  ...
  if (path) {
    ...
    const m = await evaluate('({status:vm.status,error:vm.error,serial:vm.serial.slice(-4000),commands:vm.commands})');
