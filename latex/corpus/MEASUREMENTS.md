...
`latex/corpus/packages/` asks for the four packages an ordinary paper
...
document, `latex/corpus/packages/`, which the harness does not carry and
...
`latex/corpus/packages/` asks for, in one document, in two pages, matching a
...
`latex/corpus/packages/` asks for -- compiles to its two pages through
