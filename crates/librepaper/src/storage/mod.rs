...
//! `librepaper serve` while holding no durable state of its own. The bytes,
//! the bill and the ownership of the data are the operator's, in a bucket they
...
impl StorageOptions {
    ...
    pub fn fill_from_environment(&mut self) {
        ...
        self.catalog = first_of(&[&self.catalog, &env("LIBREPAPER_CATALOG")]);
        self.catalog_token = first_of(&[&self.catalog_token, &env("LIBREPAPER_CATALOG_TOKEN")]);
        self.server_state = first_of(&[&self.server_state, &env("LIBREPAPER_SERVER_STATE")]);
        self.replica = first_of(&[&self.replica, &env("LIBREPAPER_CATALOG_REPLICA")]);
        if self.catalog_sync == 0 {
            self.catalog_sync = env("LIBREPAPER_CATALOG_SYNC").parse().unwrap_or(5);
        }
        if self.catalog_reads == CatalogReads::Replica {
            if let Ok(value) = env("LIBREPAPER_CATALOG_READS").parse() {
                self.catalog_reads = value;
            }
        }
        self.endpoint = first_of(&[&self.endpoint, &env("LIBREPAPER_S3_ENDPOINT")]);
        self.bucket = first_of(&[
            &self.bucket,
            &env("LIBREPAPER_S3_BUCKET"),
            &env("LIBREPAPER_R2_BUCKET"),
        ]);
        self.region = first_of(&[&self.region, &env("LIBREPAPER_S3_REGION"), "auto"]);
        ...
        self.prefix = first_of(&[&self.prefix, &env("LIBREPAPER_S3_PREFIX")]);
        self.access_key = first_of(&[
            &self.access_key,
            &env("LIBREPAPER_S3_ACCESS_KEY"),
            &env("AWS_ACCESS_KEY_ID"),
            &env("CLOUDFLARE_R2_ACCESS_KEY_ID"),
        ]);
        self.secret_key = first_of(&[
            &self.secret_key,
            &env("LIBREPAPER_S3_SECRET_KEY"),
    ...
    pub fn profile(&self) -> Result<(DeploymentProfile, DeploymentPaths), String> {
        ...
        if hosted_values {
            ...
            if self.catalog_token.is_empty() {
                return Err("hosted mode requires LIBREPAPER_CATALOG_TOKEN".into());
            }
        ...
        let dir = first_of(&[
            &self.dir,
            &std::env::var("LIBREPAPER_DATA").unwrap_or_default(),
            "librepaper-data",
        ]);
        let absolute =
...
pub struct StorageFlags {
    ...
    pub catalog_sync: u64,
    /// S3-compatible endpoint URL; or $LIBREPAPER_S3_ENDPOINT
    #[arg(long = "s3-endpoint", alias = "endpoint", value_name = "URL")]
    pub s3_endpoint: Option<String>,
    /// R2/S3 bucket for hosted objects; or $LIBREPAPER_R2_BUCKET
    #[arg(long = "bucket", alias = "s3-bucket", value_name = "BUCKET")]
    pub s3_bucket: Option<String>,
    /// Bucket region (default auto); or $LIBREPAPER_S3_REGION
    #[arg(long, value_name = "REGION")]
    pub s3_region: Option<String>,
    /// Explicit key prefix, so a bucket can be shared; or
    /// $LIBREPAPER_S3_PREFIX
    #[arg(long, value_name = "PREFIX")]
    pub s3_prefix: Option<String>,
    /// Prefer $LIBREPAPER_S3_ACCESS_KEY: a flag is visible in the process
    /// table
    #[arg(long, value_name = "KEY")]
    pub s3_access_key: Option<String>,
    /// Prefer $LIBREPAPER_S3_SECRET_KEY: a flag is visible in the process
    /// table
        ...
        if self.s3_access_key.as_deref().is_some_and(|v| !v.is_empty()) {
            eprintln!(
                "warning: --s3-access-key is visible to every process on this machine\n  \
                 and lands in your shell history. Prefer LIBREPAPER_S3_ACCESS_KEY."
            );
        }
        if self.s3_secret_key.as_deref().is_some_and(|v| !v.is_empty()) {
            eprintln!(
                "warning: --s3-secret-key is visible to every process on this machine\n  \
                 and lands in your shell history. Prefer LIBREPAPER_S3_SECRET_KEY."
            );
        }
