...
impl Catalog {
    ...
    /// Rebuild the canonical room comment from rows held by `tx` and hash it
    /// with the same helper used by the MCP acceptance gate.
    pub(super) fn comment_version_for_document_tx(
        ...
        let mut rows = statement
            .query(params![slug, document_id, comment_id])
            .map_err(CatalogError::from)?;
        let mut replies = Vec::new();
        while let Some(row) = rows.next().map_err(CatalogError::from)? {
            replies.push(Reply {
                slug: row.get(0).map_err(CatalogError::from)?,
                comment_id: row.get(1).map_err(CatalogError::from)?,
                id: row.get(2).map_err(CatalogError::from)?,
                body: row.get(3).map_err(CatalogError::from)?,
                creator: row.get(4).map_err(CatalogError::from)?,
                author: row.get(5).map_err(CatalogError::from)?,
                created: timestamp(row.get(6).map_err(CatalogError::from)?),
            });
        }
        crate::room::agent_comments::catalog_comment_version(comment, replies)
    }
