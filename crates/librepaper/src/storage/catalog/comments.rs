...
fn validate_new_request_key(key: &str) -> CatalogResult<()> {
    let issued = crate::util::request_key_timestamp(key)
...
fn annotation_actor(authority: AnnotationAuthority<'_>) -> String {
    if !authority.account_id.is_empty() { authority.account_id.to_string() }
    else if !authority.generation.is_empty() { format!("session:{}", authority.generation) }
    else { "anonymous".into() }
}
...
fn annotation_account_authorized(tx: &Transaction<'_>, document_id: &str, authority: AnnotationAuthority<'_>) -> CatalogResult<()> {
    if authority.account_id.is_empty() { return Ok(()); }
    let allowed: bool = tx.query_row(
        ...
        params![document_id, authority.account_id, authority.generation], |row| row.get(0),
    ).map_err(CatalogError::from)?;
    if allowed { Ok(()) } else { Err(CatalogError::refused(CatalogRefusal::ActorRights, "annotation authority changed".into())) }
}
...
impl Catalog {
    ...
    pub fn pending_suggestion_accept(&self,slug:&str,comment_id:&str)->CatalogResult<bool>{self.with_connection(|c|{let doc=document_id_connection(c,slug)?;c.query_row("SELECT EXISTS(SELECT 1 FROM operations WHERE document_id=?1 AND kind='agent_annotations' AND state='prepared' AND json_extract(plan_json,'$.commentId')=?2)",params![doc,comment_id],|r|r.get(0)).map_err(CatalogError::from)})}
    ...
    fn insert_reply_tx(tx:&Transaction<'_>,reply:&Reply,authority:AnnotationAuthority<'_>)->CatalogResult<Reply>{
        annotation_session_active(tx,authority)?; let doc=document_id(tx,&reply.slug)?; annotation_account_authorized(tx,&doc,authority)?;
