...
impl Catalog {
    ...
    /// objects in one bounded read. Room cold-start uses the physical digest
    /// recorded by the asset object, since assets do not require a separate
    /// logical digest column.
    ...
    fn commit_v2_checkpoint_for_operation(
        ...
        self.immediate(|tx| {
            ...
            if let Some(agent) = agent {
                ...
                if agent_state == "prepared" {
                    ...
                    if suggestion_accept {
                        ...
                        annotation_acceptance = Some((comment_id.to_owned(), None));
                    }
            ...
            // The CAS above checks the captured input watermark. A base written
            // in this transaction becomes the durable representation of this
            // checkpoint, so retain its resulting watermark on the checkpoint.
