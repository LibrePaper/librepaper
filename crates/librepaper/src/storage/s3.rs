...
pub struct S3Store {
    ...
    bucket: String,
    region: String,
    prefix: String, // every key librepaper writes lives under this
    access_key: String,
    secret_key: String,
    single_writer: bool,
...
impl S3Store {
    ...
    /// Puts a key under this deployment's prefix, so librepaper can share a bucket
    ...
    pub async fn probe(&self) -> ProbeReport {
        const SCRATCH: &str = ".librepaper-probe";
        let mut report = ProbeReport::default();

        let first = match self
            .write(SCRATCH, b"librepaper".to_vec(), "text/plain", &[])
            .await
        {
            Ok(first) => first,
            Err(err) => {
        ...
        if let Err(err) = self
            .write(SCRATCH, b"librepaper".to_vec(), "text/plain", &right)
            .await
        {
