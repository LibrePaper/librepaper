...
async fn invalid_explicit_session_is_not_downgraded_to_anonymous() {
    ...
    let response = client()
        ...
        .header("authorization", "Bearer lp_not-a-signed-session")
        .send()
        .await
        .unwrap();
...
async fn a_dead_session_cookie_is_cleared_and_the_listing_recovers() {
    ...
    let cookie = format!("librepaper_session={stale}");
    let clears_session = |response: &reqwest::Response| {
        response
            ...
            .iter()
            .filter_map(|v| v.to_str().ok())
            .any(|c| c.starts_with("librepaper_session=;") && c.contains("Max-Age=0"))
    };
