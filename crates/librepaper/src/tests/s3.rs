...
//! -- it is a check that what librepaper sends is what an S3 answers to, and
//! that the store reads the answers correctly.
...
async fn s3_store_against_a_bucket() {
    ...
    // Everything librepaper writes lands under its own prefix, so a bucket can
    // be shared and a clear has a bounded blast radius.
    blobs
        .put(
    ...
    assert!(
        objects
            .lock()
            .unwrap()
            .contains_key("librepaper/content/a-paper/trees/abc"),
    ...
    assert!(
        !objects
            .lock()
            .unwrap()
            .contains_key("librepaper/.librepaper-probe"),
...
fn canonical_encoding() {
    assert_eq!(
        escape_path("/librepaper/documents/a b/c.html"),
        "/librepaper/documents/a%20b/c.html"
    );
    let query = vec![
        ("b".to_string(), "2".to_string()),
        ("a".to_string(), "1".to_string()),
        ("prefix".to_string(), "librepaper/documents/".to_string()),
    ];
    assert_eq!(
        canonical_query(&query),
        "a=1&b=2&prefix=librepaper%2Fdocuments%2F"
    );
}
...
async fn a_server_backed_by_a_bucket() {
    ...
    // Everything is in the bucket, under librepaper's own prefix, in the
    // layout the store expects.
    for wanted in [
        "librepaper/index.json".to_string(),
        ...
        format!("librepaper/sessions/{slug}"),
        format!("librepaper/content/{slug}/trees/{}", text(&document, "sha")),
        format!("librepaper/history/{slug}/index.json"),
    ] {
        assert!(
    ...
    assert!(
        objects
            .lock()
            .unwrap()
            .contains_key(&format!("librepaper/rooms/{slug}.json")),
    ...
    // And deleting takes librepaper's keys and nothing else.
    objects
        .lock()
        .unwrap()
        .insert("librepaper-other-thing".into(), b"not ours".to_vec());
    let (status, _) = post(
        &server.url,
    ...
    assert!(
        held.contains_key("librepaper-other-thing"),
