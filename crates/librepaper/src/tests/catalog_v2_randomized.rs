...
async fn open(root: &Path) -> (Arc<Store>, RoomSet) {
    ...
    let store = Arc::new(Store::open_with_catalog(blobs.clone(), config.clone(), catalog).await.unwrap());
    let rooms = RoomSet::new(blobs.clone(), config);
    rooms.attach_store(store.clone());
    rooms.attach_deployment_lock(crate::server::serve::acquire_writer_lock(&root.join("state/writer.lock")).unwrap());
...
async fn audit(store: &Store) {
    ...
    for (id, key, digest, length) in objects {
        ...
        assert_eq!(hex::encode(Sha256::digest(&bytes)), digest, "physical digest: {key}");
        physical_bytes += length;
        expected_keys.insert(key);
        physical.insert(id, bytes);
    }
    let listed: BTreeSet<_> = store.blobs.list("v2/documents/").await.unwrap().into_iter().map(|entry| entry.key).collect();
    assert_eq!(listed, expected_keys, "completed mutations leave no untracked physical object");
    let points: Vec<(String, String)> = catalog.with_connection(|connection| {
        let mut statement = connection.prepare("SELECT id,tree_object_id FROM checkpoints ORDER BY seq")?;
        ...
        for file in tree.files.values() {
            ...
            if let Some(locator) = &file.recipe {
                ...
                for chunk in &recipe.chunk_locators {
                    assert_eq!(Sha256::digest(&physical[chunk.object_id.as_str()]).as_slice(), chunk.object_digest);
                    expected.insert(chunk.object_id.to_string());
                }
                let reconstructed = crate::storage::encoding::reconstruct(&recipe.recipe, |digest| {
                    let chunk = recipe.chunk_locators.iter().find(|chunk| chunk.logical_digest.as_ref() == Some(digest)).unwrap();
                    Ok(physical[chunk.object_id.as_str()].clone())
                }).unwrap();
                assert_eq!(reconstructed.len() as u64, file.logical_length);
                assert_eq!(Sha256::digest(&reconstructed).as_slice(), file.logical_digest);
            }
        }
        let actual: BTreeSet<String> = catalog.with_connection(|connection| {
            let mut statement = connection.prepare("SELECT object_id FROM checkpoint_objects WHERE checkpoint_id=?1")?;
    ...
    assert_eq!(totals, (physical_bytes, physical_bytes, physical_bytes, edge_count, edge_count));
    ...
    let journal = crate::storage::journal::V2JournalRuntime::with_persistence(
        Arc::new(crate::storage::v2_catalog::V2JournalCatalogAdapter::with_limits(catalog.clone(), store.config.persistence())),
...
async fn seeded_writes_labels_retention_gc_and_reopen_preserve_physical_closures() {
    ...
    store.put_as_actor(Publication { slug: "random-audit".into(), title: "Random audit".into(), source: "initial".into(), source_format: "markdown".into(), main: "main.md".into(), owner: "alice".into(), ..Default::default() }, super::room::fixture_actor()).await.unwrap();
    ...
    for step in 0..48 {
        ...
        match action {
            0..=2 => {
                ...
                room.checkpoint_now("quiet", "alice").await.unwrap_or_else(|error| panic!("checkpoint at randomized step {step}, action {action}: {error}"));
            }
            3 => {
                let room = rooms.get("random-audit").await;
                ...
                assert_eq!(rooms.get("random-audit").await.source().await, expected, "reopen at step {step}");
            }
            4 => {
                let catalog = store.catalog.as_ref().unwrap();
                let points = catalog.checkpoints("random-audit", None, 100).unwrap();
                let point = &points[(random as usize >> 8) % points.len()];
                catalog.label_checkpoint("random-audit", &point.sha, if point.label.is_empty() { "retained" } else { "" }).unwrap();
            }
            5 => {
                let catalog = store.catalog.as_ref().unwrap();
                ...
                catalog.schedule_document_balanced("random-audit", now, Default::default()).unwrap();
                ...
                crate::storage::maintenance_v2::run_gc_pass(&adapter, store.blobs.as_ref(), now + 87_300_002).await.unwrap();
            }
            6 => {
                let room = rooms.get("random-audit").await;
                let (digest, _) = room.put_asset_authorized(random.to_le_bytes().repeat(4), (1024, 65_536), &super::room::fixture_actor()).await.unwrap();
        ...
        assert_eq!(rooms.get("random-audit").await.source().await, expected, "live state at step {step}");
        audit(&store).await;
    }
    assert!(actions.iter().all(|count| *count > 0), "seed must exercise every action: {actions:?}");
    drop(rooms);
    drop(store);
