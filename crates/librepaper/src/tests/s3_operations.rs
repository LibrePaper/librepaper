...
async fn a_create_only_write_that_lost_its_answer_is_reconciled_not_repeated() {
    ...
    assert_eq!(
        held.objects
            .get("librepaper/recovery/manifest")
            .map(Vec::as_slice),
...
async fn a_create_only_write_that_finds_other_bytes_is_a_conflict() {
    ...
    let blobs = store(&endpoint, clock.clone());
    {
        let mut held = bucket.lock().expect("bucket");
        held.objects
            .insert("librepaper/recovery/manifest".into(), b"theirs".to_vec());
...
async fn a_partial_failure_inside_a_successful_response_is_reported_per_key() {
    ...
    let blobs = store(&endpoint, clock.clone());
    {
        ...
        held.batch_replies.push_back(
            "<DeleteResult>\
             <Deleted><Key>librepaper/one</Key></Deleted>\
             <Error><Key>librepaper/two</Key><Code>NoSuchKey</Code><Message>gone</Message></Error>\
             <Error><Key>librepaper/three</Key><Code>InternalError</Code><Message>later</Message></Error>\
             <Error><Key>librepaper/four</Key><Code>AccessDenied</Code><Message>no</Message></Error>\
             </DeleteResult>"
                .into(),
        );
        held.batch_replies.push_back(
            "<DeleteResult><Deleted><Key>librepaper/three</Key></Deleted></DeleteResult>".into(),
        );
    }
    ...
    bucket.lock().expect("bucket").batch_replies.push_back(
        "<DeleteResult><Error><Key>librepaper/four</Key><Code>AccessDenied</Code><Message>no</Message></Error></DeleteResult>".into(),
    );
