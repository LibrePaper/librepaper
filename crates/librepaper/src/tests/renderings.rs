...
async fn put_current_rendering(
    ...
    let response = client()
        .put(format!("{base}/api/documents/{slug}/renderings/{name}"))
        .header("x-librepaper-client", "1")
        .header("x-librepaper-current", "1")
        .header("x-librepaper-inputs", inputs)
...
async fn latest_says_which_rendering_and_whether_it_is_current() {
    ...
    // The name a browser compiling the text now would store its pages under,
    // said before any rendering exists: that is when a browser needs it.
    let sha = live_sha(&server, &slug).await;
    assert_eq!(text(&answer, "live"), sha, "{answer}");
    put_rendering(
        &session_as(TEST_PUBLISHER),
        &server.url,
        &slug,
    ...
    assert_eq!(
        text(&answer, "live"),
        live_sha(&server, &slug).await,
        "the live name did not move with the text"
    );
    assert_ne!(
        text(&answer, "live"),
        sha,
        "the text moved and the live name did not"
    );
}
