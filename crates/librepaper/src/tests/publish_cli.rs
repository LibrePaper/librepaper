//! What `librepaper publish` sends from a directory: what .gitignore excludes,
...
fn sync_lock_files_are_not_publishable() {
    ...
    std::fs::write(root.path().join("main.md.librepaper-lock"), "1234\n").unwrap();
