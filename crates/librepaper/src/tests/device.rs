...
//! `lp_` token that comes out the other end.
...
/// The whole flow, as `librepaper login` walks it, with the approval made by a
/// forged GitHub session. The stand-in for GitHub's check-token endpoint is
/// there to be counted rather than called: an `lp_` bearer must never reach
/// it.
#[tokio::test]
async fn a_terminal_signs_in_through_the_deployment() {
    ...
    assert_eq!(
        ...
        0,
        "an lp_ token went to GitHub, and it must be verified locally"
    );
...
/// An `lp_` bearer is only as good as its signature and its expiry, exactly
...
async fn a_forged_or_stale_device_token_is_nobody() {
    ...
    assert_eq!(
        github.hits.load(std::sync::atomic::Ordering::Relaxed),
        0,
        "an lp_ token that failed locally was then taken to GitHub"
    );
}

/// The other bearer, unchanged: anything without the prefix is a GitHub token
/// and goes through check-token, which is what keeps `LIBREPAPER_TOKEN`
/// working.
#[tokio::test]
    ...
    let router = axum::Router::new()
        ...
        .route(
            ...
            post(|State(polls): State<Arc<AtomicUsize>>| async move {
                ...
                axum::Json(json!({"token": "lp_a_token", "expires_in": 100}))
            }),
        )
