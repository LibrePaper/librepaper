...
pub struct Room {
    ...
    /// Serializes a complete Quarto publication with the Quarto retention
    /// sweep. Per-object rendering locks are insufficient because a bundle
    /// stages several blobs before its manifest is committed.
    pub(crate) quarto_publication: Mutex<()>,
    /// Coalesces cleanup requests generated by bursty bundle publication
    /// failures. The sweep clears this flag after it has released its locks.
    pub(crate) quarto_cleanup_pending: AtomicBool,
...
impl RoomSet {
    ...
    pub async fn get(&self, slug: &str) -> Arc<Room> {
        ...
        let room = Arc::new(Room {
            ...
            quarto_publication: Mutex::new(()),
            quarto_cleanup_pending: AtomicBool::new(false),
...
impl Room {
    ...
    async fn swap_accounted_with_type(
        &self,
        key: &str,
        body: Vec<u8>,
        _content_type: &str,
        ...
        let selection = if key.contains("/selections/") {
            serde_json::from_slice::<crate::quarto::Selection>(&body).ok()
        } else {
            None
        };
        let change = ObjectChange {
        ...
        let reservation = reservation.map(|guard| match selection.clone() {
            Some(selection) => guard.with_quarto_selection(selection),
            None => guard,
        });
        let previous = match self.blobs.get_versioned(key).await {
            Ok((body, version)) => Some((body, version)),
            Err(BlobError::NotFound) => None,
            Err(error) => {
                if let Some(reservation) = reservation {
                    reservation.abort().await;
                }
                return Err(WriteError::Storage(error.to_string()));
            }
        };
        // A committed pointer remains authoritative even if a crash or
        // backend repair lost its transport blob. Recreate the cache under
        // an empty expectation; the pointer transaction below still fences
        // the new selection.
        let cas_expect = if previous.is_none() {
            ""
        } else if selection.is_some() && expect.is_empty() {
            // A cleared pointer may leave only an uncommitted transport blob.
            // Replace that cache using its actual CAS version; the catalog
            // pointer remains the authority for whether the result is read.
            previous
                .as_ref()
                .map_or(expect, |(_, version)| version.as_str())
        } else {
            expect
        };
        match self.blobs.swap(key, body, cas_expect).await {
            Ok(version) => {
                if let Some(reservation) = reservation {
                    if let Err(error) = reservation.commit(version.clone()).await {
                        // The catalogue authority is checked again by commit.
                        // If revocation won during the blob await, remove the
                        // unselected object or restore the prior selection so
                        // no unauthorized metadata becomes readable.
                        let rollback = match previous {
                            Some((old_body, _old_version)) => {
                                self.blobs.swap(key, old_body, &version).await.map(|_| ())
                            }
                            None => {
                                self.blobs
                                    .delete(std::slice::from_ref(&key.to_string()))
                                    .await
                            }
                        };
                        if let Err(rollback_error) = rollback {
                            eprintln!(
                                "warning: could not roll back fenced Quarto object {key}: {rollback_error}"
                            );
                        }
            ...
            Err(BlobError::Conflict) => {
                ...
                Err(WriteError::Conflict(
                    "Quarto publication generation changed".into(),
                ))
            }
            Err(error) => {
                if let Some(reservation) = reservation {
