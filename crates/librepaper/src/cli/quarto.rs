...
async fn connection(
    ...
    let server = super::server_from(&server.unwrap_or_default());
    let key = super::link_key(&key.unwrap_or_default());
    let credentials = Credentials::new(&super::stored_token_for(&server), &key);
    let slug = super::resolve_identifier(id, &server, &key).await;
...
pub async fn run(command: QuartoCommand) -> Result<(), String> {
    match command {
        ...
        QuartoCommand::Status {
            ...
            let (server, slug, who) = connection(&id, server, key).await;
