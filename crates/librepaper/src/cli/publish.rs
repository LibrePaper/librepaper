...
pub fn files_under(root: &Path, main_stem: &str, ignored: &dyn Fn(&Path) -> bool) -> Vec<String> {
    ...
    while let Some(directory) = stack.pop() {
        ...
        for entry in entries {
            ...
            if name.starts_with('.') || name.ends_with(".librepaper-lock") {
                continue;
            }
...
fn quarto_shared_paths(root: &Path, main: &str, files: Vec<String>) -> Result<Vec<String>, String> {
    ...
    let include = match read_policy {
        Ok(bytes) => {
            ...
            serde_json::from_slice::<Share>(&bytes)
                .map_err(|err| format!("invalid .librepaper-share.json: {err}"))?
                .include
        }
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => Vec::new(),
        Err(err) => return Err(format!("cannot read .librepaper-share.json: {err}")),
    };
    ...
    let quarto_stems: std::collections::BTreeSet<_> = files
        .iter()
...
pub(super) async fn publish_file(file: &str, mut title: String, slug: String, server_flag: String) {
    ...
    // Every format librepaper renders has one, HTML included: its renderer is
    ...
    if is_typst(file) {
        ...
        // Nothing is rendered here, and nothing can be: LibrePaper carries no
        // TeX and no build embeds one. So this is the one format `publish`
        // uploads without having compiled it first, and the check that a
        // document compiles -- which is the reason `publish` compiles typst at
        // all -- happens in the first browser that opens it instead. Said
        // plainly, because an author used to `publish` refusing a broken paper
        // should not have to infer that this one is different.
        if title.is_empty() {
