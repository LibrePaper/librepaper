...
/// `<cache_home>/komodoc/local/jobs/<id>/`, never under a project directory
...
async fn start(port: u16, foreground: bool) {
    if !foreground {
        println!(
            "note: --foreground has no effect yet; komodoc local always stays attached to \
    ...
    if let Some(existing) = pairing.read_service() {
        if existing.port == port && process_alive(existing.pid) {
            die(format!(
                "komodoc local is already running on port {} (pid {}). Use `komodoc local \
    ...
    println!(
        "komodoc local listening on http://127.0.0.1:{port}{}",
        protocol::BASE_PATH
    );
    ...
    println!("komodoc local stopped");
}

async fn status() {
    let pairing = PairingStore::new(&config_home());
    let Some(state) = pairing.read_service() else {
        println!("komodoc local is not running");
        return;
    };
    let url = format!(
...
async fn rescan() {
    ...
    // `<config_home>/komodoc/local/tools.json`, which a running service
