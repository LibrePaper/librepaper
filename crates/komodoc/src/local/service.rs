//! The loopback HTTP surface `komodoc local start` binds:
...
struct Inner {
    ...
    work: Notify,
    /// `<cache_home>/komodoc/local/jobs`; each job gets `<jobs>/<id>/`.
    jobs_root: PathBuf,
...
impl LocalService {
    ...
    pub fn new(
        ...
        let jobs_root = cache_home.join("komodoc").join("local").join("jobs");
...
fn handle_health(inner: &Inner) -> Reply {
    let body = protocol::Health {
        service: "komodoc-local".to_string(),
