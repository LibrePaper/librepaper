...
//!
//! State lives under `<config_home>/komodoc/local/` -- a sibling of, not
//! inside, the deployment token cache `crate::cli` keeps under
//! `<config_home>/komodoc/` -- as two small JSON files:
//!
//! - `service.json`: what the running `komodoc local start` printed, so
...
/// What `komodoc local start` writes about itself, so a later command in a
...
impl PairingStore {
    ...
    pub fn new(config_home: &Path) -> Self {
        PairingStore {
            dir: config_home.join("komodoc").join("local"),
        }
    }
    ...
    /// The code a `connect` must present: `$KOMODOC_LOCAL_CODE` when set --
    ...
    pub fn expected_code(&self) -> String {
        if let Ok(code) = std::env::var("KOMODOC_LOCAL_CODE") {
    ...
    /// pairing this store holds. What `komodoc local disconnect` does.
...
/// A fresh six-digit pairing code, as `komodoc local start` prints and
...
mod tests {
    ...
    fn origin_normalisation_ignores_case_and_matches_port() {
        assert_eq!(
            normalize_origin("HTTPS://Komodoc.Example:5173/"),
            normalize_origin("https://komodoc.example:5173")
        );
        assert_ne!(
            normalize_origin("https://komodoc.example"),
            normalize_origin("https://komodoc.example:5173")
        );
    }
}
