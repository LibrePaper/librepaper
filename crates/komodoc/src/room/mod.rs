...
/// The validated comment protocol.  `Message` remains the wire-compatible
/// adapter, but room code receives one of these operation-specific values so
/// fields belonging to another operation cannot accidentally participate in
/// validation, accounting, or request identity.
...
/// A wire command that cannot enter room accounting or mutation processing.
...
impl CommandError {
    ...
    pub fn response(&self) -> Value {
        let (temp_id, request_id) = match self {
            Self::Unknown {
                temp_id,
                request_id,
                ..
...
impl Message {
    /// Converts the compatible wire representation into a command before any
    /// operation-specific processing. Raw strings are retained byte-for-byte
    /// so request digests and durable retry receipts keep their identity.
