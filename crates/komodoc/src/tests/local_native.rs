...
fn tex_request(engine: &str, main: &str, deadline_seconds: u64, max_passes: u32) -> JobRequest {
    JobRequest {
        ...
        origin: "https://komodoc.example".to_string(),
...
async fn shell_escape_and_path_escape_attempts_are_refused() {
    let (workspace, _guard) = stage_from_source(
        r"\documentclass{article}
\begin{document}
\immediate\write18{touch /tmp/komodoc-native-test-pwned}
\newwrite\myout
\immediate\openout\myout=../komodoc-native-test-escape.txt
...
",
    ...
    let marker = std::path::Path::new("/tmp/komodoc-native-test-pwned");
    let _ = std::fs::remove_file(marker);
    let escape_target = workspace.root.join("komodoc-native-test-escape.txt");
