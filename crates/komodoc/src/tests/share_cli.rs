//! Tests for the share_cli work of the link-based sharing model: `komodoc
...
//! `$KOMODOC_TOKEN` or the on-disk cache (see `require_token_for`), neither of
//! which a unit test should be poking at process-wide state to fake. What it
//! does with the answer, though, is an ordinary function of a JSON value in
//! the shape `sharing_json` on the server answers with, so these tests build
//! that shape by hand and drive `format_role_row`, `sharing_report_lines`,
//! and the local `--link` role check directly -- the same way
//! `review_cli.rs` tests `files_under` and `store_token_at` without standing
//! up a server.
