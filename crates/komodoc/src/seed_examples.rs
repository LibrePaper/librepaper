...
//! document per source format Komodoc accepts. Komodoc's own engine renders
//! the .md and the .typ at seed time, the HTML one is what Quarto produced
//! from its .qmd and is stored as it arrived, and the .tex is stored as
//! source and compiled by the browser that opens it, because nothing here
//! carries a TeX.
//!
...
pub fn seed_documents() -> Vec<SeedDocument> {
    vec![
        ...
        SeedDocument {
            ...
            // beside this file, figures inlined, and Komodoc stores the page
            // as its own source. It is the shape most papers arrive in, and
            // the one with figures for a region annotation to be drawn on.
