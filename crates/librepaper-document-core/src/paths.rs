...
//! of file sits at it.
//!
//! One function, asked by every route that takes a path, by the projection
//! that reads what a peer wrote into the shared document, and by the command
//! line before it sends anything -- so a name refused on the server is
...
impl Kind {
    /// The spelling a tree entry and a projection digest use.
